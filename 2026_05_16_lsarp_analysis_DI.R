...
v.date = "2026_05_15" # saved with all projects included (some processing, analysis, proteomics, UC)
# save.image("./2026_05_15_lsarp_analysis_di.Renv")
# save.image("./2026_08_11_lsarp_analysis_di.Renv") # revised dissertation data

# load("./2026_08_11_lsarp_analysis_di.Renv")
# note: 01_15 is missing UC and potentially more
...
lsarp.cd.deid = read.csv("./lsarp_plots_deidentified/2026_05_12_lsarp_cd_codes.csv")
...
# used for nearly all figures
rs.colors.type = c("Potato" = "#F8766D", 
              "Green banana" = "#A3A500", "Corn" = "#00BF7D", 
              "Cross-linked" = "#00B0F6", "Cross-linked potato" = "#E76BF3")
# 
...
metadata.lsarp.stool.plot = ggplot(metadata.lsarp.stool %>% 
  ...
  geom_point(fill = "white", aes(shape=lsarp.on.rs), size=5)+
  # then plot real points (they overlap in the window, but look good when saved at full size)
  geom_point(aes(fill=lsarp.on.rs, shape=lsarp.on.rs), color="white",
  ...
  scale_shape_manual(values=c(4, 21, 21, 23))+
  ...
  # deid
  scale_y_discrete(labels = lsarp.cd.deid$code, 
                   breaks = lsarp.cd.deid$HM) +
...
lsarp.delta.omic.lmer = function(data = lsarp.lmer.asv.data,
                                 ...
                                 mpx = F, # add "plate" random effect for metaproteomics
                                 split = FALSE) {
  
  meta.cols = c("HM","group","phase","timing", "standard.name", "lsarp.days","rs.end", "ave.fiber", "plate")
    ...
    if(split == FALSE){
      lmer.output = do.call(rbind, lapply(c("treatment", "washout"), function(p){
       if(p == "washout"){
        ...
        if(sum(prev.total)>0){
          # 
          # for metaproteomic data:
          if(mpx == T){
            lmer.output = lmerTest::lmer((feature) ~ lsarp.days*group + (1|HM) + (1|plate), 
                                         data.response) %>% summary() %>% coef()
            #
            lmer.output = data.frame(lmer.output)[which(rownames(lmer.output) == "lsarp.days:groupRS"),
                                                  c(1,5)] # take last row corresponding to interaction
            lmer.output$taxa = x
...
lsarp.delta.omic.resp.lmer = function(data = lsarp.lmer.asv.data,
  ...
  meta.cols = c("HM","group","phase","timing", "standard.name", "lsarp.days","rs.end", "ave.fiber", "flare.group", "plate")
  ...
  lm.output = do.call(rbind, lapply((colnames(data)[!colnames(data) %in% meta.cols]), function(x){
    ...
    if(split == FALSE){
      lmer.output = do.call(rbind, lapply(unique(lsarp.metadata.responders$group), function(p){
        ...
        if(sum(prev.total)>0){
          # for meta-proteomic, control for plate as a covariate
          if(mpx == T){
            #
            if(scale == T){
            lmer.output = lmerTest::lmer((feature) ~ (lsarp.days)*flare.group + (1|HM) + (1|plate), 
                                         data.response %>%
                                           mutate(feature = scale(feature),
                                                  lsarp.days = scale(lsarp.days))) %>% summary() %>% coef()
            } else{
              lmer.output = lmerTest::lmer((feature) ~ lsarp.days*flare.group + (1|HM) + (1|plate), 
                                           data.response) %>% summary() %>% coef()
            }
            #
            lmer.output = data.frame(lmer.output)[which(rownames(lmer.output) == "lsarp.days:flare.groupRemit"),c(1,5)]
            lmer.output$taxa = x
            colnames(lmer.output)[1] = "estimate"
            colnames(lmer.output)[2] = "pval"
            lmer.output$group = p
            ...
            # for non-metaproteomic, do not control for metaproteomic plate
            if(scale == T){
              lmer.output = lmerTest::lmer((feature) ~ (lsarp.days)*flare.group + (1|HM), 
                                           data.response %>%
                                             mutate(feature = scale(feature),
                                                    lsarp.days = scale(lsarp.days))) %>% summary() %>% coef()
...
lsarp.delta = function(data = lsarp.fecalcal.omics.data[,colnames(lsarp.fecalcal.omics.data)%in% c(subset(metadata.lsarp.stool.omics.cor.bh, padj < 0.20)$feature)],
  ...
  if(type == "trajectory"){
    ...
    data.output = do.call(rbind, lapply(1:(ncol(data)), function(col){
      ...
      data.subset = merge(data.subset, metadata.lsarp.stool.asv[,c("HM", "standard.name", "lsarp.days", "phase", "rs.end", "plate")], by="standard.name")
  ...
  # calculate all Log2FC to baseline
    ...
    lfc.output = do.call(rbind, lapply(lmer.sig.features, function(feature){
      if(responders == F){
        data.subset = lsarp.lmer.omics[,colnames(lsarp.lmer.omics) %in% c(feature, "standard.name","HM","RS_Name", "Group", "lsarp.days", "baseline", "plate")]
      }
      if(responders == T){
        data.subset = lsarp.lmer.omics[,colnames(lsarp.lmer.omics) %in% c(feature, "standard.name","HM","RS_Name", "Group", "lsarp.days", "baseline", "flare.group", "plate")]
      }
...
pval.positioner = function(
    variable = "fcal",
    data = metadata.lsarp.stool,
    data1 = lsarp.stats.fcal.treatment,
    data2 = lsarp.stats.fcal.washout,
    type = "group", # or "response"
    yadjust = 1.1){
  if(is.null(variable)){
  ...
  treatment.range = c(0, max(subset(data, phase == "treatment" | phase == "Treatment")$lsarp.days))
  placebo.range = c(max(subset(data, phase == "treatment" | phase == "Treatment")$lsarp.days), max(subset(data,  phase == "washout" | phase == "Washout")$lsarp.days))
  # 2. calculate center value (mean)
  treatment.center = mean(treatment.range)
  washout.center = mean(placebo.range)
  # 3. calculate y value, add 5%
  y.val = max(na.omit(data[,variable]))*yadjust
  # 4. save coords as data.frame
    output = data.frame(phase = c("Treatment", "Washout"),
               xval = c(treatment.center, washout.center),
               yval = y.val,
               label = c(paste("𝘱:", round(data.frame(data1)[rownames(data1) == "groupRS:scale(lsarp.days)",5], digits=3)),
                         paste("𝘱:", round(data.frame(data2)[rownames(data2) == "groupRS:scale(lsarp.days)",5], digits=3))))
  # add dummy for Placebo
    output = rbind(output %>% mutate(group = "RS"),
                   data.frame(phase = c("Treatment", "Washout"),
                              xval = NA,
                              yval = NA,
                              label = c(NA,NA)) %>% mutate(group = "Placebo")) %>% data.frame()
    }
  if(type == "response"){
    # 1. calculate x ranges for each phase:
    range = range(subset(data, phase == "treatment")$lsarp.days)
    # 2. calculate center value (mean)
    treatment.center = mean(range)
    # 3. calculate y value, add 5%
    y.val = max(na.omit(data[,variable]))*yadjust
    # save coords
    output = data.frame(group = c("Placebo", "RS"),
               xval = c(treatment.center),
               yval = y.val,
               label = c(paste("𝘱:", round(data.frame(data1)[rownames(data1) == "scale(lsarp.days):flare.groupRemit",5], digits=3)),
                         paste("𝘱:", round(data.frame(data2)[rownames(data2) == "scale(lsarp.days):flare.groupRemit",5], digits=3))))
  }
  return(output)
}
...
# Note: We don't have longitudinal data, so this analysis cannot be done 
...
lsarp.cd.rs.selections.rs.plot <- ggplot(subset(lsarp.cd.rs.selections,group=="RS"),
  ...
  labs(x="", y=" ", title="RS")+
  facet_wrap(~HM,
             labeller = labeller(HM = setNames(lsarp.cd.deid$code, lsarp.cd.deid$HM)))+
  theme_minimal()+theme(legend.position="none",
...
lsarp.cd.rs.selections.frequences.plot = ggplot(lsarp.cd.rs.selections.frequences %>%
                                                  ...
                                                  rbind(data.frame(Selected = "FibersymRW", group="RS", Group = "RS", Freq = 0, perc = 0)),
...
mean(subset(metadata.lsarp.stool, lsarp.on.rs == "pre.rs")$delta.ph)
...
lsarp.baseline.multiome.permanova = 
  rbind(
    ...
    as.data.frame(lsarp.baseline.cazy.pca.permanova)[1,] %>% mutate(type = "CAZyme"),
    as.data.frame(lsarp.baseline.mbx.pca.permanova)[1,] %>% mutate(type = "Metabolite"))

ggplot(lsarp.baseline.multiome.permanova %>%
         mutate(type = factor(type, levels=c("ASV", "Species", "Pathway", "COG", "CAZyme", "Metabolite"))),
...
# add required variables for analysis
...
# remove 3 patients without dose; nope, not true ITT if you remove them
# redcap.lsarp.wpcdai.itt = subset(redcap.lsarp.wpcdai, !group %in%  c("(RS)", "(Plac)"))
redcap.lsarp.wpcdai.itt = redcap.lsarp.wpcdai
subset(redcap.lsarp.wpcdai.itt, is.na(time.to.event))
redcap.lsarp.wpcdai.itt$group = ifelse(redcap.lsarp.wpcdai.itt$group == "(RS)", "RS",
                                       ifelse(redcap.lsarp.wpcdai.itt$group == "(Plac)", "Plac",
                                              redcap.lsarp.wpcdai.itt$group))
redcap.lsarp.wpcdai.itt$time.to.event = ifelse(is.na(redcap.lsarp.wpcdai.itt$time.to.event), 0, 
                                               redcap.lsarp.wpcdai.itt$time.to.event)
redcap.lsarp.wpcdai.itt$status.pre = ifelse(redcap.lsarp.wpcdai.itt$HM %in% c("HM1045", "HM0966", "HM0970", "HM0974"), 1,
                                            redcap.lsarp.wpcdai.itt$status.pre)


# first question is: "is remission more likely in patients on RS up to scope"
redcap.lsarp.survival.itt <- survival::survfit(Surv(time.to.event, status.pre) ~ group, 
                                           data = redcap.lsarp.wpcdai.itt)
redcap.lsarp.log.rank.test.itt <- survival::survdiff(Surv(time.to.event, status.pre) ~ group, 
                                                 data = redcap.lsarp.wpcdai.itt,
                                                 rho = 0)

redcap.lsarp.coxph.itt = survival::coxph(Surv(time.to.event, status.pre) ~ group, data = redcap.lsarp.wpcdai.itt)
redcap.lsarp.coxph.itt <- broom::tidy(redcap.lsarp.coxph.itt, exponentiate = TRUE, conf.int = TRUE) %>%
  mutate(HR = round(estimate, 3),
         `95% CI` = paste0(round(conf.low, 3), " - ", round(conf.high, 3)),
         `p-value` = signif(p.value, 3)) %>%
  dplyr::select(term, HR, `95% CI`, `p-value`) %>% as.data.frame()

redcap.lsarp.log.rank.test.itt
# not significant

redcap.lsarp.survival.itt.plot = survminer::ggsurvplot(redcap.lsarp.survival.itt, 
                                                   data = redcap.lsarp.wpcdai.itt,
                                                   pval = F,             # show log-rank p-value
                                                   conf.int = F,         # show confidence intervals
                                                   risk.table = TRUE,       # show number at risk
                                                   conf.int.style="ribbon",
                                                   xlab = "Days since starting Product",
                                                   ylab = "Cumulative event of relapse",
                                                   pval.size=4,
                                                   censor.shape=124,
                                                   censor.size=3,
                                                   legend="right",
...
# Subset to remove non-compliant
redcap.lsarp.wpcdai.pp = subset(redcap.lsarp.wpcdai, HM %in% metadata.lsarp.stool$HM)
nrow(redcap.lsarp.wpcdai.pp)
# n = 26, good

# ALSO REMOVE HM0962; 68% compliance in placebo (keep in microbiome analyses)

# numbers
redcap.lsarp.wpcdai.pp[,c("HM", "group")] %>% distinct() %>% dplyr::select(group) %>% table()

# first question is: "is remission more likely in patients on RS up to scope"
redcap.lsarp.survival.pp <- survival::survfit(Surv(time.to.event, status.pre) ~ group, 
                                 data = subset(redcap.lsarp.wpcdai.pp, HM != "HM0962"))
redcap.lsarp.log.rank.test.pp <- survival::survdiff(Surv(time.to.event, status.pre) ~ group, 
                       data = subset(redcap.lsarp.wpcdai.pp, HM != "HM0962"),
                       rho = 0)

redcap.lsarp.coxph.pp = survival::coxph(Surv(time.to.event, status.pre) ~ group, 
                                        data = subset(redcap.lsarp.wpcdai.pp, HM != "HM0962"))
redcap.lsarp.coxph.pp <- broom::tidy(redcap.lsarp.coxph.pp, exponentiate = TRUE, conf.int = TRUE) %>%
  mutate(HR = round(estimate, 3),
         `95% CI` = paste0(round(conf.low, 3), " - ", round(conf.high, 3)),
         `p-value` = signif(p.value, 3)) %>%
  dplyr::select(term, HR, `95% CI`, `p-value`) %>% as.data.frame()

redcap.lsarp.log.rank.test.pp
# not significant

redcap.lsarp.survival.pp.plot = survminer::ggsurvplot(redcap.lsarp.survival.pp, 
           data = subset(redcap.lsarp.wpcdai.pp, HM != "HM0962"),
           pval = F,             # show log-rank p-value
           conf.int = F,         # show confidence intervals
           risk.table = TRUE,       # show number at risk
           conf.int.style="ribbon",
           xlab = "Days since starting Product",
           ylab = "Cumulative event of relapses",
           pval.size=4,
           censor.shape=124,
           censor.size=3,
           legend="right",
...
# ITT
lsarp.itt.plot = redcap.lsarp.survival.itt.plot$plot + 
  ylim(1,0.5)+
  scale_y_reverse(limits = c(1, 0.5), 
                  labels = scales::percent(rev(seq(0, 0.5, by = 0.1))))+
  labs(title="Intention-to-treat (ITT)")+theme(plot.title = element_text(hjust = 0.5))

lsarp.itt.plots = lsarp.itt.plot/
  redcap.lsarp.survival.itt.plot$table+#labs(title=NULL)+
  patchwork::plot_layout(heights=c(4,1))


# PP
lsarp.pp.plot = redcap.lsarp.survival.pp.plot$plot + 
  ylim(1,0.5)+
  scale_y_reverse(limits = c(1, 0.5), 
                  labels = scales::percent(rev(seq(0, 0.5, by = 0.1))))+
  labs(title="Per-protocol (PP)")+theme(plot.title = element_text(hjust = 0.5))

lsarp.pp.plots = lsarp.pp.plot/
  redcap.lsarp.survival.pp.plot$table+#labs(title=NULL)+
  patchwork::plot_layout(heights=c(4,1))

# combined

cowplot::plot_grid(lsarp.itt.plots, lsarp.pp.plots) %>%
  ggsave(filename="./lsarp_plots/2026_07_27_lsarp_itt_pp_km.pdf",
         width=12, height=5,device = cairo_pdf)

# HRs
redcap.lsarp.coxph.itt #  HR = 0.856 (0.292 - 2.592)
redcap.lsarp.coxph.pp #   HR = 0.396 (0.094 - 1.662)

# pvalues
redcap.lsarp.log.rank.test.itt$pvalue # p = 0.744
redcap.lsarp.log.rank.test.pp$pvalue  # p = 0.190
...
# check wPCDAI, fCal, alpha diversity, and butyrogens at baseline
# PP analysis

# wPCDAI (note: THIS IS AT TIME OF FLARE)
redcap.lsarp.wpcdai.pp
subset(redcap.lsarp.wpcdai.pp, HM %in% metadata.lsarp.stool$HM) %>% nrow()
# n = 26, good
lsarp.baseline.wpcdai.test = wilcox.test(as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% metadata.lsarp.stool$HM & group == "RS")$wpcdai.other),
            as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% metadata.lsarp.stool$HM & group == "Plac")$wpcdai.other))
# p = 0.027
# plot

lsarp.baseline.wpcdai.test.plot = redcap.lsarp.wpcdai.pp %>%
  mutate(Group = ifelse(group == "Plac", "Placebo", "RS")) %>%
  mutate(Group = factor(Group, levels=c("Placebo", "RS"))) %>%
  # take average of first 2, since several data points are missing
  ggplot(aes(x=Group, y=as.numeric(wpcdai.other)))+
  geom_boxplot(width=0.5)+
  #scale_y_log10()+
  ggbeeswarm::geom_beeswarm(shape=21, color="white", 
                            aes(fill=Group), size=3)+
  geom_text(data = data.frame(pval = lsarp.baseline.wpcdai.test$p.value),
            aes(x=1.5, y=Inf, label = paste("𝘱: ", round(pval, digits=3), sep="")), 
            vjust=1.3, size=4)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text = element_text(size=12),
                        strip.background = element_rect(
                          color="black"))+
  labs(x=NULL, y="Time of Flare wPCDAI")
lsarp.baseline.wpcdai.test.plot


# SES AT TIME OF FLARE
redcap.lsarp.wpcdai.pp
subset(redcap.lsarp.wpcdai.pp, HM %in% metadata.lsarp.stool$HM& ses.other != "Not done") %>% nrow()
# n = 24
lsarp.baseline.ses.test = wilcox.test(as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% metadata.lsarp.stool$HM & group == "RS" & ses.other != "Not done")$ses.other),
                                         as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% metadata.lsarp.stool$HM & group == "Plac"& ses.other != "Not done")$ses.other))
# p = 0.5035


# fCal
subset(metadata.lsarp.stool, baseline == "baseline" & !HM %in% excluded.by.dave) %>% nrow()
# n = 26, good

lsarp.baseline.fcal.test = wilcox.test(as.numeric(subset(metadata.lsarp.stool, baseline == "baseline" & !HM %in% excluded.by.dave & group == "RS")$fcal),
                                         as.numeric(subset(metadata.lsarp.stool, baseline == "baseline" & !HM %in% excluded.by.dave & group == "Placebo")$fcal))
# p = 0.654
# plot

lsarp.baseline.fcal.test.plot = subset(metadata.lsarp.stool, baseline == "baseline" & !HM %in% excluded.by.dave) %>%
  mutate(Group = factor(Group, levels=c("Placebo", "RS"))) %>%
  # take average of first 2, since several data points are missing
  ggplot(aes(x=Group, y=as.numeric(fcal)))+
  geom_boxplot(width=0.5)+
  scale_y_log10()+
  ggbeeswarm::geom_beeswarm(shape=21, color="white", 
                            aes(fill=group), size=3)+
  geom_text(data = data.frame(pval = lsarp.baseline.fcal.test$p.value),
            aes(x=1.5, y=Inf, label = paste("𝘱: ", round(pval, digits=3), sep="")), 
            vjust=1.3, size=4)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x=NULL, y="Fecal Calprotectin (μg/g)")
lsarp.baseline.fcal.test.plot


# Richness
lsarp.baseline.richness.test = wilcox.test(as.numeric(subset(metadata.lsarp.stool.asv, baseline == "baseline" & !HM %in% excluded.by.dave & group == "RS")$richness),
                                       as.numeric(subset(metadata.lsarp.stool.asv, baseline == "baseline" & !HM %in% excluded.by.dave & group == "Placebo")$richness))
# p = 0.527

# Shannon
lsarp.baseline.shannon.test = wilcox.test(as.numeric(subset(metadata.lsarp.stool.asv, baseline == "baseline" & !HM %in% excluded.by.dave & group == "RS")$shannon),
                                           as.numeric(subset(metadata.lsarp.stool.asv, baseline == "baseline" & !HM %in% excluded.by.dave & group == "Placebo")$shannon))
# p = 0.160

# But
lsarp.baseline.but.i.test = wilcox.test(as.numeric(subset(metadata.lsarp.stool.asv, baseline == "baseline" & !HM %in% excluded.by.dave & group == "RS")$but.i),
                                           as.numeric(subset(metadata.lsarp.stool.asv, baseline == "baseline" & !HM %in% excluded.by.dave & group == "Placebo")$but.i))
# p = 0.462
...
lsarp.new.table = read.csv("~/Documents/PhD/From_Dave/2026_08_02_lsarp_baseline_characteristics.csv")

lsarp.new.table = lsarp.new.table[,colnames(lsarp.new.table) %in% c("HM", "Group","Response", "age_at_endo","wpcdai_at_rs", "bmi_at_endo", "M", "upper_gi..L4.", "ileal..L1.", "colonic..L2.", "ileocolonic..L3.", "inflammatory..B1.", "stricturing..B2.", "penetrating..B3.", "perianal", "wpcdai", "ses", "oral_prednisone", "IV.corticosteroids", "budesonide", "methotrexate", "azathioprine", "crp", "fcal")]

vars = c("age_at_endo","wpcdai_at_rs", "bmi_at_endo", "M", "upper_gi..L4.", "ileal..L1.", "colonic..L2.", "ileocolonic..L3.", "inflammatory..B1.", "stricturing..B2.", 
         "perianal", "wpcdai", "ses", "oral_prednisone", "IV.corticosteroids", "budesonide", "methotrexate", "azathioprine", "crp", "fcal")

# run through stats (wilcoxon vs Fishers)
lsarp.baseline.stats = do.call(rbind, lapply(vars, function(x){
  print(x)
  # for numerical, use wilcoxon
  if(x %in% c("age_at_endo","wpcdai_at_rs", "bmi_at_endo", "wpcdai", "ses", "crp", "fcal")){
    # select columns
    data.subset = lsarp.new.table[, colnames(lsarp.new.table) %in% c("Group", x)]
    test.result = wilcox.test(subset(data.subset, Group == "RS")[,2],
                              subset(data.subset, Group == "Plac")[,2])
    # pval
    pval = test.result$p.value
    # median
    median.rs = median(na.omit(subset(data.subset, Group == "RS")[,2]))
    median.plac = median(na.omit(subset(data.subset, Group == "Plac")[,2]))
    # range
    range.rs = range(na.omit(subset(data.subset, Group == "RS")[,2]))
    range.plac = range(na.omit(subset(data.subset, Group == "Plac")[,2]))
    # save df
    save.df = data.frame(
      variable = x,
      pval = pval,
      median.rs = median.rs,
      median.plac = median.plac,
      range.rs.1 = range.rs[1],
      range.rs.2 = range.rs[2],
      range.plac.1 = range.plac[1],
      range.plac.2 = range.plac[2],
      num.rs = NA,
      num.plac = NA,
      perc.rs = NA,
      perc.plac = NA
  ...
  # for categorical, fishers
  if(!x %in% c("age_at_endo","wpcdai_at_rs", "bmi_at_endo", "wpcdai", "ses", "crp", "fcal")){
    # select columns
    data.subset = lsarp.new.table[, colnames(lsarp.new.table) %in% c("Group", x)]
    colnames(data.subset)[2] = "var"
    data.subset = data.subset %>%
      mutate(var = ifelse(is.na(var), 0, 1))
    test.result = fisher.test(table(data.subset))
    # pval
    pval = test.result$p.value
    # %
    perc.rs = sum(subset(data.subset, Group == "RS")[,2]) / nrow(subset(data.subset, Group == "RS")) * 100
    perc.plac = sum(subset(data.subset, Group == "Plac")[,2]) / nrow(subset(data.subset, Group == "Plac")) * 100
    
    # #
    num.rs = sum(subset(data.subset, Group == "RS")[,2])
    num.plac = sum(subset(data.subset, Group == "Plac")[,2])
...
lsarp.baseline.stats

write.csv(lsarp.baseline.stats,
          "~/Documents/PhD/From_Dave/2026_08_02_lsarp_baseline_stats.csv")

# change in wpcdai over induction

lsarp.new.table %>%
  mutate(change.wpcdai = (wpcdai - wpcdai_at_rs)) %>%
  #subset(Group == "RS") %>%
  ggplot(aes(x=Response, y=wpcdai_at_rs))+
  geom_boxplot()+ geom_jitter() 

# check responders
wilcox.test(subset(lsarp.new.table, Group == "RS" & Response == "no")$wpcdai_at_rs,
            subset(lsarp.new.table, Group == "RS" & Response == "yes")$wpcdai_at_rs)
# p = 0.3865 (overall)
# p = 0.1853 (RS)
# p = 0.5769 (Placebo)
...
lsarp.new.table %>%
  mutate(change.wpcdai = (wpcdai - wpcdai_at_rs)) %>%
  ggplot(aes(x=Group, y=change.wpcdai))+
  geom_boxplot()+ geom_jitter() 
...
metadata.lsarp.fcal.plot = ggplot(metadata.lsarp.stool %>%
  ...
  geom_text(data=pval.positioner("fcal",
                                 metadata.lsarp.stool,
                                 lsarp.stats.fcal.treatment,
                                 lsarp.stats.fcal.washout,
                                 type="group",
                                 yadjust=2), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text = element_text(size=10),
...
metadata.lsarp.water.plot = ggplot(metadata.lsarp.stool %>%
  ...
  geom_text(data=pval.positioner("stool_water_perc",
                                 metadata.lsarp.stool,
                                 lsarp.stats.water.treatment,
                                 lsarp.stats.water.washout,
                                 type="group",
                                 yadjust=1.05), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
...
metadata.lsarp.load.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  geom_text(data=pval.positioner("load.asv",
                                 ...
                                 lsarp.stats.load.treatment,
                                 lsarp.stats.load.washout,
                                 type="group",
                                 yadjust=(1.01)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.stool.asv.richness.plot = ggplot(metadata.lsarp.stool.asv %>%
 ...
 # scale_y_log10()+
  ...
  geom_text(data=pval.positioner("richness",
                                 metadata.lsarp.stool.asv,
                                 lsarp.stats.richness.treatment,
                                 lsarp.stats.richness.washout,
                                 type="group",
                                 yadjust=(1.0)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.stool.asv.shannon.plot = ggplot(metadata.lsarp.stool.asv  %>%
  ...
  geom_text(data=pval.positioner("shannon",
                                 metadata.lsarp.stool.asv,
                                 lsarp.stats.shannon.treatment,
                                 lsarp.stats.shannon.washout,
                                 type="group",
                                 yadjust=(1.05)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.stool.asv.fr.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  geom_text(data=pval.positioner("fr",
                                 metadata.lsarp.stool.asv,
                                 lsarp.stats.fr.treatment,
                                 lsarp.stats.fr.washout,
                                 type="group",
                                 yadjust=(1.0)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.stool.asv.fd.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  geom_text(data=pval.positioner("fd",
                                 metadata.lsarp.stool.asv,
                                 lsarp.stats.fd.treatment,
                                 lsarp.stats.fd.washout,
                                 type="group",
                                 yadjust=(1.02)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.but.i.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  geom_text(data=pval.positioner("but.i",
                                 metadata.lsarp.stool.asv,
                                 lsarp.stats.but.i.treatment,
                                 lsarp.stats.but.i.washout,
                                 type="group",
                                 yadjust=(1.1)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  scale_y_continuous(labels=scales::percent_format(accuracy=1))+
...
# try controlling for baseline wPCDAI
redcap.lsarp.wpcdai.pp
# stats
lmerTest::lmer(scale(log10(but.i)) ~ group*scale(lsarp.days) + ave.fiber + wpcdai + (1|HM),
               subset(metadata.lsarp.stool.asv, phase == "treatment")  %>%
                 merge(redcap.lsarp.wpcdai.pp[,c("HM", "wpcdai.other")], by="HM") %>%
                 mutate(wpcdai = as.numeric(wpcdai.other)) %>%
                 # remove samples collected after flare
                 subset(flare != "flare" & !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
# no impact, not sig
...
metadata.lsarp.but.ii.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  geom_text(data=pval.positioner("but.ii",
                                 metadata.lsarp.stool.asv,
                                 lsarp.stats.but.ii.treatment,
                                 lsarp.stats.but.ii.washout,
                                 type="group",
                                 yadjust=(1.05)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  scale_y_continuous(labels=scales::percent_format(accuracy=1))+
...
lsarp.asv.pcoa.plot = ggplot(
  ...
  labs(x=paste("Axis 1: ", round(unique(lsarp.asv.pcoa.df$var1), digits=2), "%", sep=""), 
       ...
       title=paste(paste("ASV\nTreatment R²: ", round(data.frame(lsarp.asv.permanova)[2,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.asv.permanova)[2,5], 3), sep="")), sep="") + 
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text = element_text(size=10),
...
metadata.lsarp.stool.asv.between.plot = ggplot(beta.trajectory.data  %>%
  ...
  geom_text(data=pval.positioner("between.beta",
                                 ...
                                 stats.beta.between.treatment,
                                 stats.beta.between.washout,
                                 type="group",
                                 yadjust=(1.01)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.stool.asv.within.plot = ggplot(beta.trajectory.data  %>%
  ...
  geom_text(data=pval.positioner("within.beta",
                                 beta.trajectory.data,
                                 stats.beta.within.treatment,
                                 stats.beta.within.washout,
                                 type="group",
                                 yadjust=(1.01)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
lsarp.lmer.asv.interactions = lsarp.delta.omic.lmer(lsarp.lmer.asv.data,
                                                    split=F)
# fix names
lsarp.lmer.asv.interactions$feature =  ifelse(grepl("s__", (lsarp.lmer.asv.interactions$taxa)), paste("(s) ", gsub("g__", "", gsub("f__", "", gsub("s__", "", (lsarp.lmer.asv.interactions$taxa)))), sep=""),
...
lsarp.lmer.asv.interactions.plot = ggplot(lsarp.lmer.asv.interactions %>%
  ...
  ggrepel::geom_text_repel(aes(label=ifelse(padj < 0.20, gsub("\\..*", "", feature), NA)),
                           size=2.5)+
  ggrepel::geom_text_repel(aes(label=ifelse(pval < 0.05 & padj >0.2, gsub("\\..*", "", feature), NA)),
                           size=2.5, color="grey")+
...
stats.beta.between.treatment
stats.beta.between.washout
lsarp.stats.starch.treatment
lsarp.stats.starch.washout
lsarp.stats.mucin.treatment
lsarp.stats.mucin.washout
...
lsarp.mgx.pcoa.plot <- ggplot(
  ...
  labs(x=paste("Axis 1: ", round(unique(lsarp.mgx.pcoa.df$var1), digits=2), "%", sep=""), 
       ...
       title=paste(paste("Species\nTreatment R²: ", round(data.frame(lsarp.mgx.permanova)[2,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.mgx.permanova)[2,5], 3), sep="")), sep="") + 
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text = element_text(size=10),
...
set.seed(25)
t1 = Sys.time()
lsarp.kegg.lsarp.pca.permanova = vegan::adonis2(dist(lsarp.kegg.lsarp.pca$x[,c(1,2)]) ~ group*on.rs + ave.fiber + plate,
                                                lsarp.kegg.lsarp.pca.df %>%
                                                  mutate(on.rs = as.factor(ifelse(lsarp.on.rs=="on.rs", "on.rs", "off.rs"))),
...
lsarp.kegg.lsarp.pca.plot <- ggplot(
  ...
  labs(x=paste("Axis 1: ", round((lsarp.kegg.lsarp.pca.var)[1], digits=2), "%", sep=""), 
       ...
       title=paste(paste("Pathway\nTreatment R²: ", round(data.frame(lsarp.kegg.lsarp.pca.permanova)[3,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.kegg.lsarp.pca.permanova)[3,5], 3), sep="")), sep="")
...
# adding a plate random effect improves p values
...
set.seed(25)
t1 = Sys.time()
lsarp.cog.lsarp.pca.permanova = vegan::adonis2(dist(lsarp.cog.lsarp.pca$x) ~ group*on.rs +ave.fiber+plate,
                                                 lsarp.cog.lsarp.pca.df %>%
                                                 mutate(on.rs = as.factor(ifelse(lsarp.on.rs=="on.rs", "on.rs", "off.rs"))),
...
lsarp.cog.lsarp.pca.plot <- ggplot(
  ...
  labs(x=paste("Axis 1: ", round((lsarp.cog.lsarp.pca.var)[1], digits=2), "%", sep=""), 
       ...
       title=paste(paste("COG\nTreatment R²: ", round(data.frame(lsarp.cog.lsarp.pca.permanova)[3,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.cog.lsarp.pca.permanova)[3,5], 3), sep="")), sep="")
lsarp.cog.lsarp.pca.plot


# :: Plate effects --------------------------------------------------------

# Here I present evidence that "plate" has a significant impact on proteomic composition, 
# and therefore needs to controlled for in stats. 
# I went back and reformulated code to include plate as a random effect in lmer, and fixed effect in PERMANOVA (since that's the best you can do) 

# permanova
set.seed(25)
t1 = Sys.time()
lsarp.cog.lsarp.pca.plate.permanova = vegan::adonis2(dist(lsarp.cog.lsarp.pca$x) ~ plate,
                                                     lsarp.cog.lsarp.pca.df,
                                                     strata = lsarp.cog.lsarp.pca.df$HM,
                                                     by="margin")
t2 = Sys.time()
t2 - t1
lsarp.cog.lsarp.pca.plate.permanova
# therefore, ought to include as covariate
...
set.seed(25)
t1 = Sys.time()
lsarp.cazy.lsarp.pca.permanova = vegan::adonis2(dist(lsarp.cazy.lsarp.pca$x) ~ group*on.rs +ave.fiber+plate,
                                                lsarp.cazy.lsarp.pca.df %>%
                                                  mutate(on.rs = as.factor(ifelse(lsarp.on.rs=="on.rs", "on.rs", "off.rs"))),
...
lsarp.cazy.lsarp.pca.plot <- ggplot(
  ...
  labs(x=paste("Axis 1: ", round((lsarp.cazy.lsarp.pca.var)[1], digits=2), "%", sep=""), 
       ...
       title=paste(paste("CAZyme\nTreatment R²: ", round(data.frame(lsarp.cazy.lsarp.pca.permanova)[3,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.cazy.lsarp.pca.permanova)[3,5], 3), sep="")), sep="")
...
lsarp.lmer.mpx.cazy.interactions.plot = ggplot(lsarp.lmer.mpx.cazy.interactions%>%
  ...
  ggrepel::geom_text_repel(aes(label=ifelse(pval < 0.05 & padj >0.2, gsub("\\..*", "", taxa), NA)),
                           size=2.5, color="grey")+
  labs(x="Interaction Coefficient",
       title="Group Interaction (CAZyme)",
       y="FDR")+
  facet_wrap(~phase)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
...
# stats
lsarp.stats.starch.treatment = lmerTest::lmer(scale(log10(starch)) ~ group*scale(lsarp.days) + ave.fiber + (1|HM) + (1|plate),
                                                    subset(metadata.lsarp.stool.asv, phase == "treatment")  %>%
                                                      # remove samples collected after flare
                                                      subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.stats.starch.washout = lmerTest::lmer(scale(log10(starch)) ~ group*scale(lsarp.days) + ave.fiber + (1|HM) + (1|plate),
                                                  subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")  %>%
                                              # remove samples collected after flare
                                              subset(flare != "flare"& !HM %in% excluded.by.dave))  %>%
  summary() %>% coef()
...
metadata.lsarp.starch.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  labs(x="Days since starting Product", y="Starch CAZymes")+
...
# stats
lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days) + ave.fiber + (1|HM) + (1|plate),
                                              subset(metadata.lsarp.stool.asv, 
...
# stats
lsarp.stats.mucin.treatment = lmerTest::lmer(scale(log10(mucin)) ~ group*scale(lsarp.days) + ave.fiber +(1|HM)+(1|plate),
                                                    subset(metadata.lsarp.stool.asv, phase == "treatment")  %>%
                                               # remove samples collected after flare
                                               subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.stats.mucin.washout = lmerTest::lmer(scale(log10(mucin)) ~ group*scale(lsarp.days) + ave.fiber + (1|HM) + (1|plate),
                                                  subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")  %>%
                                             # remove samples collected after flare
                                             subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
...
metadata.lsarp.mucin.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  scale_fill_manual(values=rs.colors, na.value="grey")+
  # geom_text(aes(label=HM), size=3, vjust=-1)+
  labs(x="Days since starting Product", y="Mucin CAZymes")+
...
# stats
lsarp.stats.starch.mucin.treatment = lmerTest::lmer(scale((starch.mucin)) ~ group*scale(lsarp.days) + ave.fiber +(1|HM)+(1|plate),
                                             subset(metadata.lsarp.stool.asv, phase == "treatment")  %>%
                                               # remove samples collected after flare
                                               subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%  summary() %>% coef()

lsarp.stats.starch.mucin.washout = lmerTest::lmer(scale((starch.mucin)) ~ group*scale(lsarp.days) + ave.fiber +(1|HM)+(1|plate),
                                           subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")  %>%
                                             # remove samples collected after flare
                                             subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
...
metadata.lsarp.starch.mucin.plot = ggplot(metadata.lsarp.stool.asv %>%
  ...
  geom_text(data=pval.positioner("starch.mucin",
                                 metadata.lsarp.stool.asv,
                                 lsarp.stats.starch.mucin.treatment,
                                 lsarp.stats.starch.mucin.washout,
                                 type="group",
                                 yadjust=(1.0)), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group,scales="free_x")+
...
colnames(lsarp.mbx.annotated.mat)[grepl("thia", colnames(lsarp.mbx.annotated.mat))]
colnames(lsarp.mbx.raw.mat.filt.1)[grepl("hia", colnames(lsarp.mbx.raw.mat.filt.1))]
# no thiamine
...
lsarp.mbx.lsarp.pca.plot <- ggplot(
  ...
  labs(x=paste("Axis 1: ", round((lsarp.mbx.lsarp.pca.var)[1], digits=2), "%", sep=""), 
       ...
       title=paste(paste("Metabolite\nTreatment R²: ", round(data.frame(lsarp.mbx.lsarp.pca.permanova)[2,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.mbx.lsarp.pca.permanova)[2,5], 3), sep="")), sep="")
...
lsarp.lmer.omics.lfc.treatment.feature.map = rbind(subset(lsarp.lmer.asv.interactions, padj < 0.20 & phase == "treatment") %>% mutate(data.type = "ASV"),
                                                   ...
                                                   subset(lsarp.lmer.mpx.cazy.interactions, padj < 0.20 & phase == "treatment") %>% mutate(data.type = "CAZyme"),
...
lsarp.metadata.responders = readRDS("./2025_09_10_lsarp_cd_response_list.Rds")
...
redcap.lsarp.wpcdai.filtered = redcap.lsarp.wpcdai.filtered[,c("group","flare.call", "HM", "wpcdai.other", "crp.other", "fcal.other")] %>%
  as.data.frame() %>%
...
lsarp.metadata.responders = merge(lsarp.metadata.responders,
                                  redcap.lsarp.wpcdai.filtered[,c("HM", "condition", "wpcdai.other", "crp.other", "fcal.other")],
                                  by="HM")
...
# :: Time of Flare wPCDAI ------------------------------------------------------

# wPCDAI AT TIME OF FLARE
lsarp.resp.baseline.wpcdai.test.rs = wilcox.test(as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Relapse" & group == "RS")$HM)$wpcdai.other),
                                         as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Remit" & group == "RS")$HM)$wpcdai.other))
# p = 0.075; relapsers had higher wPCDAI (mean 12 vs mean 1)

lsarp.resp.baseline.wpcdai.test.plac = wilcox.test(as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Relapse" & group == "Placebo")$HM)$wpcdai.other),
                                              as.numeric(subset(redcap.lsarp.wpcdai.pp, HM %in% subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Remit" & group == "Placebo")$HM)$wpcdai.other))
# p = 0.0488; relapsers had higher wPCDAI (mean 26 vs 0)

lm(wpcdai.other ~ group.x*flare.group,
               merge(redcap.lsarp.wpcdai.pp,lsarp.metadata.responders.ttest.fcal, by="HM")) %>%
  summary()
# remitters have lower wPCDAI at TIME OF FLARE
# (maybe not after FDR)

# OR

glm(bin.response ~ as.numeric(wpcdai.other),
    data = subset(merge(redcap.lsarp.wpcdai.pp,lsarp.metadata.responders.ttest.fcal, by="HM"),
                  HM %in% subset(lsarp.metadata.responders.ttest.fcal, group == "RS" | group == "Placebo")$HM) %>%
      mutate(bin.response = ifelse(flare.group == "Relapse", 1, 0)),
  ...
  mutate(term = x)
# combined RS + Placebo --> higher wPCDAI == relapse OR 1.48, p = 0.070
# just RS --> higher wPCDAI == relapse OR 1.32, p = 0.161


# :: Baseline Fecal Cal -----------------------------------------------------------


# are there clinical features that predict response
lsarp.metadata.responders.ttest.fcal = lsarp.metadata.responders %>%
  group_by(HM) %>%
  # take first (non-NA) measurement per HM
  subset(!is.na(fcal)) %>%
  slice_max(order_by=-lsarp.days, n=1) %>%
  mutate(mean.fcal = mean(na.omit(fcal))) %>%
  mutate(mean.days = mean(lsarp.days)) %>%
  dplyr::select(flare.group, mean.fcal, mean.days, HM, group) %>% distinct()

lsarp.metadata.responders.ttest.fcal.pval = data.frame(
  group = c("Placebo", "RS"),
  pval = c(
    wilcox.test(subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Relapse" & group == "Placebo")$mean.fcal,
                subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Remit" & group == "Placebo")$mean.fcal)$p.value,
    # placebo is not sig
    wilcox.test(subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Relapse" & group == "RS")$mean.fcal,
                subset(lsarp.metadata.responders.ttest.fcal, flare.group=="Remit" & group == "RS")$mean.fcal)$p.value))
# RS is not sig

# mean time of fcal = 25 days before starting RS
mean(lsarp.metadata.responders.ttest.fcal$mean.days)

# plot
lsarp.metadata.responders.ttest.fcal.plot = lsarp.metadata.responders.ttest.fcal %>%
  # take average of first 2, since several data points are missing
  ggplot(aes(x=flare.group, y=mean.fcal))+
  geom_boxplot(width=0.5)+
  scale_y_log10()+
  ggbeeswarm::geom_beeswarm(shape=21, color="white", 
                            aes(fill=flare.group), size=3)+
  geom_hline(yintercept=250, color="red")+
  geom_text(data = lsarp.metadata.responders.ttest.fcal.pval,
            aes(x=1.5, y=Inf, label = paste("𝘱: ", round(pval, digits=3), sep="")), 
            vjust=1.3, size=4)+
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x=NULL, y="Initial Fecal Calprotectin (μg/g)")
lsarp.metadata.responders.ttest.fcal.plot
...
# are there clinical features that predict response
lsarp.metadata.responders.ttest.but = lsarp.metadata.responders.asv %>%
  group_by(HM) %>%
  # take first (non-NA) measurement per HM
  subset(!is.na(but.i)) %>%
  slice_max(order_by=-lsarp.days, n=1) %>%
  mutate(mean.but.i = mean(na.omit(but.i))) %>%
  mutate(mean.days = mean(lsarp.days)) %>%
  dplyr::select(flare.group, mean.but.i, mean.days, HM, group) %>% distinct()
lsarp.metadata.responders.ttest.but.pval = data.frame(
  group = c("Placebo", "RS"),
  pval = c(
wilcox.test(subset(lsarp.metadata.responders.ttest.but, flare.group=="Relapse" & group == "Placebo")$mean.but.i,
            subset(lsarp.metadata.responders.ttest.but, flare.group=="Remit" & group == "Placebo")$mean.but.i)$p.value,
# placebo is not sig
wilcox.test(subset(lsarp.metadata.responders.ttest.but, flare.group=="Relapse" & group == "RS")$mean.but.i,
            subset(lsarp.metadata.responders.ttest.but, flare.group=="Remit" & group == "RS")$mean.but.i)$p.value))
# RS is not sig

# mean time of fcal = 31 days before starting RS
mean(lsarp.metadata.responders.ttest.but$mean.days)

# plot
lsarp.metadata.responders.ttest.but.plot = lsarp.metadata.responders.ttest.but %>%
  # take average of first 2, since several data points are missing
  ggplot(aes(x=flare.group, y=mean.but.i))+
  geom_boxplot(width=0.5)+
  scale_y_log10()+
  ggbeeswarm::geom_beeswarm(shape=21, color="white", 
                            aes(fill=flare.group), size=3)+
  geom_text(data = lsarp.metadata.responders.ttest.but.pval,
            aes(x=1.5, y=Inf, label = paste("𝘱: ", round(pval, digits=3), sep="")), 
                vjust=1.3, size=4)+
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x=NULL, y="Initial Butyrogens (%)")
lsarp.metadata.responders.ttest.but.plot
...
lsarp.metadata.responders.ttest.fiber.pval = data.frame(
  group = c("Placebo", "RS"),
  pval = c(
    wilcox.test(distinct(subset(lsarp.metadata.responders, flare.group=="Relapse" & group == "Placebo")[,c("HM", "ave.fiber")])$ave.fiber,
                distinct(subset(lsarp.metadata.responders, flare.group=="Remit" & group == "Placebo")[,c("HM", "ave.fiber")])$ave.fiber)$p.value,
    # placebo is not sig
    wilcox.test(distinct(subset(lsarp.metadata.responders, flare.group=="Relapse" & group == "RS")[,c("HM", "ave.fiber")])$ave.fiber,
                distinct(subset(lsarp.metadata.responders, flare.group=="Remit" & group == "RS")[,c("HM", "ave.fiber")])$ave.fiber)$p.value))
  # RS is not sig

# plot
lsarp.metadata.responders.ttest.fiber.plot = lsarp.metadata.responders %>%
  dplyr::select(HM, flare.group, group, ave.fiber) %>% distinct() %>%
  # take average of first 2, since several data points are missing
  ggplot(aes(x=flare.group, y=ave.fiber))+
  geom_boxplot(width=0.5)+
  scale_y_log10()+
  ggbeeswarm::geom_beeswarm(shape=21, color="white", 
                            aes(fill=flare.group), size=3)+
  geom_text(data = lsarp.metadata.responders.ttest.fiber.pval,
            aes(x=1.5, y=Inf, label = paste("𝘱: ", round(pval, digits=3), sep="")), 
            vjust=1.3, size=4)+
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x=NULL, y="Average Fiber Intake (g / 1000 kcal / d)")
lsarp.metadata.responders.ttest.fiber.plot

# fiber intake does not predict response to RS


# :: RS type --------------------------------------------------------------

lsarp.metadata.responders %>%
  dplyr::select(HM, group, flare.group, RS_Name) %>% distinct() %>%
  mutate(RS_Name = factor(RS_Name, levels=rs.names)) %>%
  ggplot(aes(x=flare.group, y=RS_Name))+
  ggbeeswarm::geom_beeswarm(shape=21, color="white",
                            aes(fill=RS_Name), size=3)+
  scale_fill_manual(values=rs.colors)+
  theme_classic()+theme(legend.position="none")+
  facet_wrap(~group)+
  labs(x="Response", y=NULL)
# no consistent differences in RS selections

lsarp.metadata.responders %>%
  dplyr::select(HM, group, flare.group, RS_Name, delta.ph) %>% distinct() %>%
  mutate(RS_Name = factor(RS_Name, levels=rs.names)) %>%
  ggplot(aes(x=flare.group, y=delta.ph))+
  ggbeeswarm::geom_beeswarm(shape=21, color="white",
                            aes(fill=RS_Name), size=3)+
  scale_fill_manual(values=rs.colors)+
  theme_classic()+theme(legend.position="none")+
  facet_wrap(~group)+
  labs(x="Response", y=NULL)
# no difference in RS responders
...
metadata.lsarp.fcal.resp.plot = ggplot(lsarp.metadata.responders  %>%
  ...
  geom_text(data=pval.positioner("fcal",
                                 metadata.lsarp.stool,
                                 data1=lsarp.resp.stats.fcal.placebo,
                                 data2=lsarp.resp.stats.fcal.treatment,
                                 type="response",
                                 yadjust=2), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
# try controlling for baseline wPCDAI
lmerTest::lmer(scale(log10(fcal)) ~ scale(lsarp.days)*flare.group + ave.fiber + wpcdai + (1|HM),
...
metadata.lsarp.water.resp.plot = ggplot(lsarp.metadata.responders %>%
  ...
  geom_text(data=pval.positioner("stool_water_perc",
                                 metadata.lsarp.stool,
                                 data1=lsarp.resp.stats.water.placebo,
                                 data2=lsarp.resp.stats.water.treatment,
                                 type="response",
                                 yadjust=1.02), 
...
metadata.lsarp.load.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("load.asv",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.load.placebo,
                                 data2=lsarp.resp.stats.load.treatment,
                                 type="response",
                                 yadjust=1.001), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
...
metadata.lsarp.stool.asv.richness.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("richness",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.richness.resp.placebo,
                                 data2=lsarp.resp.stats.richness.resp.treatment,
                                 type="response",
                                 yadjust=1.001), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
...
metadata.lsarp.stool.asv.shannon.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("shannon",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.shannon.placebo,
                                 data2=lsarp.resp.stats.shannon.treatment,
                                 type="response",
                                 yadjust=1.00), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
...
metadata.lsarp.stool.asv.fd.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("fd",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.fd.placebo,
                                 data2=lsarp.resp.stats.fd.treatment,
                                 type="response",
                                 yadjust=1.01), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
...
metadata.lsarp.stool.asv.fr.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("fr",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.fr.placebo,
                                 data2=lsarp.resp.stats.fr.treatment,
                                 type="response",
                                 yadjust=1.05), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.but.i.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("but.i",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.but.i.placebo,
                                 data2=lsarp.resp.stats.but.i.treatment,
                                 type="response",
                                 yadjust=1.055), 
            aes(x=xval, y=yval*100, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
...
# sensitivity:
# test RS resp vs Placebo resp

# stats
lmerTest::lmer(scale(log10(but.i)) ~ scale(lsarp.days)*group + ave.fiber + (1|HM),
               subset(lsarp.metadata.responders.asv, flare.group %in% c("Remit")) %>%
                                                    subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
# p = 0.027

lmerTest::lmer(scale(log10(but.i)) ~ scale(lsarp.days)*group + ave.fiber + (1|HM),
               subset(lsarp.metadata.responders.asv, flare.group %in% c("Relapse")) %>%
                 subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
# p = 0.406

# try controlling for baseline wPCDAI
lmerTest::lmer(scale(log10(but.i)) ~ scale(lsarp.days)*flare.group + ave.fiber + wpcdai + (1|HM),
                                                  subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                    subset(flare != "flare"& !HM %in% excluded.by.dave) %>%
  merge(redcap.lsarp.wpcdai.pp[,c("HM", "wpcdai.other")], by="HM") %>%
  mutate(wpcdai = as.numeric(wpcdai.other))) %>%
...
metadata.lsarp.but.ii.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("but.ii",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.but.ii.placebo,
                                 data2=lsarp.resp.stats.but.ii.treatment,
                                 type="response",
                                 yadjust=1.055), 
            aes(x=xval, y=yval*100, label=label), vjust=1,size=4)+
    facet_wrap(~group,scales="free_x")+
...
lsarp.asv.resp.pcoa.plot <- ggplot(
  ...
  labs(x=paste("Axis 1: ", round(unique(lsarp.asv.resp.pcoa.df$var1), digits=2), "%", sep=""), 
       ...
       title=paste(paste("Response:Group:Time R²: ", round(data.frame(lsarp.asv.resp.permanova)[2,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.asv.resp.permanova)[2,5], 3), sep="")), sep="") + 
...
metadata.lsarp.between.beta.resp.plot = ggplot(beta.trajectory.resp.data %>%
  ...
  labs(x="Days since starting Product", y="Bray-Curtis Dissimilarity")+
       geom_text(data=pval.positioner("between.beta",
                                      beta.trajectory.resp.data %>% mutate(phase = "treatment"),
                                      data1=lsarp.resp.stats.between.beta.placebo,
                                      data2=lsarp.resp.stats.between.beta.treatment,
                                      type="response",
                                      yadjust=1.001), 
                 aes(x=xval, y=yval*100, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.within.beta.resp.plot = ggplot(beta.trajectory.resp.data %>%
  ...
  labs(x="Days since starting Product", y="Bray-Curtis Dissimilarity")+
       geom_text(data=pval.positioner("within.beta",
                                      beta.trajectory.resp.data %>% mutate(phase = "treatment"),
                                      data1=lsarp.resp.stats.within.beta.placebo,
                                      data2=lsarp.resp.stats.within.beta.treatment,
                                      type="response",
                                      yadjust=1.05), 
                 aes(x=xval, y=yval*100, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
# fix names
lsarp.lmer.asv.resp.interactions$feature =  ifelse(grepl("s__", (lsarp.lmer.asv.resp.interactions$taxa)), paste("(s) ", gsub("g__", "", gsub("f__", "", gsub("s__", "", (lsarp.lmer.asv.resp.interactions$taxa)))), sep=""),
...
subset(lsarp.lmer.mpx.kegg.resp.interactions, padj < 0.20 & group == "Placebo")
...
# increased PTS system, ethanolamine, propanediol
subset(lsarp.lmer.mpx.cog.resp.interactions, padj < 0.20)[,c("taxa")] %>% table() %>%
  data.frame() %>% subset(Freq >1)

# repeat with scale on for Beta coefficient
lsarp.lmer.mpx.cog.resp.interactions.scaled = lsarp.delta.omic.resp.lmer(lsarp.lmer.mpx.cog.data[rownames(lsarp.lmer.mpx.cog.data) %in% stools.not.flare,],
                                                                  split=F,
                                                                  scale = T,
                                                                  mpx = T)
lsarp.lmer.mpx.cog.resp.interactions.scaled
...
lsarp.lmer.mpx.cazy.resp.interactions.plot = ggplot(lsarp.lmer.mpx.cazy.resp.interactions,
  ...
  ggrepel::geom_text_repel(aes(label=ifelse(pval < 0.05 & padj >0.2, gsub("\\..*", "", taxa), NA)),
                           size=2.5, color="grey")+
  labs(x="Interaction Coefficient",
       title="Response Interaction (CAZyme)",
       y="FDR")+
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
...
# :: CAZyme Starch Interaction -------------------------------------------------------

# stats
lsarp.resp.stats.starch.treatment = lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                         subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                     subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.starch.placebo = lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                       subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                   subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
...
metadata.lsarp.starch.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("starch",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.starch.placebo,
                                 data2=lsarp.resp.stats.starch.treatment,
                                 type="response",
                                 yadjust=1.75), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group,scales="free_x")+
...
# stats
lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days) + ave.fiber + (1|HM) + (1|plate),
               subset(lsarp.metadata.responders.asv, 
                      phase == "treatment" & group == "RS" & flare.group=="Remit")  %>%
                 # remove samples collected after flare
                 subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
# no sig increase

# :: CAZyme Mucin Interaction -------------------------------------------------------

# stats
lsarp.resp.stats.mucin.treatment = lmerTest::lmer(scale(log10(mucin)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                         subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                    subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.mucin.placebo = lmerTest::lmer(scale(log10(mucin)) ~ scale(lsarp.days)*flare.group + ave.fiber  + (1|HM)+ (1|plate),
                                                       subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                  subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
...
metadata.lsarp.mucin.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("mucin",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.mucin.placebo,
                                 data2=lsarp.resp.stats.mucin.treatment,
                                 type="response",
                                 yadjust=1.0), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group,scales="free_x")+
...
# :: CAZyme Starch:Mucin Interaction -------------------------------------------------------

# stats
lsarp.resp.stats.starch.mucin.treatment = lmerTest::lmer(scale((starch.mucin)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                   subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                     subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.starch.mucin.placebo = lmerTest::lmer(scale((starch.mucin)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                 subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                   subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()
...
metadata.lsarp.starch.mucin.resp.plot = ggplot(lsarp.metadata.responders.asv %>%
  ...
  geom_text(data=pval.positioner("starch.mucin",
                                 metadata.lsarp.stool.asv,
                                 data1=lsarp.resp.stats.starch.mucin.placebo,
                                 data2=lsarp.resp.stats.starch.mucin.treatment,
                                 type="response",
                                 yadjust=1.3), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group,scales="free_x")+
...
lsarp.lmer.omics.lfc.resp.treatment.feature.map = rbind(subset(lsarp.lmer.asv.resp.interactions, padj < 0.20 & group == "RS") %>% mutate(data.type = "ASV"),
                                                   ...
                                                   subset(lsarp.lmer.mpx.cazy.resp.interactions, padj < 0.20 & group == "RS") %>% mutate(data.type = "CAZyme"),
  ...
  mutate(data.type = ifelse(variable %in% colnames(lsarp.mgx.taxa), "Species",
                            ifelse(variable %in% colnames(lsarp.cd.mpx.kegg.mat), "Pathway", 
                                   ifelse(variable %in% colnames(lsarp.cd.mpx.cog.mat), "COG", 
                                          ifelse(variable %in% colnames(lsarp.cd.mpx.cazy.mat), "CAZyme", 
...
lsarp.fecalcal.omics.cor.df$data.type = factor(lsarp.fecalcal.omics.cor.df$data.type, levels=c("Fecal calprotectin", "ASV", "Species", "COG", "Pathway", "CAZyme", "Metabolite"))
...
data.sets = c("ASV", "Species", "Pathway", "COG", "CAZyme","Metabolite", "Multi-Omic")
                            ...
                            ifelse(feature %in% colnames(lsarp.mgx.baseline), "Species",
                                   ifelse(feature %in% colnames(lsarp.kegg.baseline), "Pathway",
                                          ifelse(feature %in% colnames(lsarp.cog.baseline), "COG",
                                                 ifelse(feature %in% colnames(lsarp.cazy.baseline), "CAZyme", 
...
lsarp.rf.models.importances.wilcox$data.type = factor(lsarp.rf.models.importances.wilcox$data.type, levels=c("ASV", "Species", "Pathway", "COG", "CAZyme", "Metabolite"))
...
lsarp.rf.models.importances.wilcox.plot = ggplot(lsarp.rf.models.importances.wilcox,
  ...
  ggnetwork::geom_nodetext_repel(
    ...
    size=2)+
 # facet_wrap(~"Log2FC ~ Wilcoxon 𝘱 value")+
...
lsarp.rf.models.importances.ttest.plot = ggplot(lsarp.rf.models.importances.ttest %>%
  ...
  geom_text(data=lsarp.rf.models.importances.ttest.p %>%
            ...
            aes(label = paste("𝘱:", round(wilcox.pval, digits=3))),
            size=2.5)+
...
lsarp.rf.validation.output = rf.function(data.types = c("ASV", "Species", "Pathway", "COG", "CAZyme","Metabolite", "Multi-Omic"),
                                         iters = 15,
                                         reduce_features = F,
                                         # p.threshold = 0.10,
                                         output = "AUC")
...
lsarp.lmer.asv.pcoa.plot <- ggplot(
  ...
  annotate(geom="text", x=0, y=Inf, hjust=0.5, vjust=1.5,
           label=paste(paste("R²: ", round(data.frame(lsarp.lmer.asv.pcoa.permanova)[1,3], 3)*100, "%",
                       "  𝘱: ", round(data.frame(lsarp.lmer.asv.pcoa.permanova)[1,5], 3), sep=""), sep=""))+
...
set.seed(25)
t1 = Sys.time()
lsarp.lmer.mgx.pcoa.permanova = vegan::adonis2(lsarp.lmer.mgx.bray ~ flare.group,
...
lsarp.lmer.mgx.pcoa.plot <- ggplot(
  ...
  annotate(geom="text", x=0, y=Inf, hjust=0.5, vjust=1.5,
           label=paste(paste("R²: ", round(data.frame(lsarp.lmer.mgx.pcoa.permanova)[1,3], 3)*100, "%",
                             "  𝘱: ", round(data.frame(lsarp.lmer.mgx.pcoa.permanova)[1,5], 3), sep=""), sep=""))+
...
set.seed(25)
t1 = Sys.time()
lsarp.lmer.kegg.pca.permanova = vegan::adonis2(dist(lsarp.lmer.kegg.pca$x[,c(1,2)]) ~ flare.group + plate,
                                               lsarp.lmer.kegg.pca.df %>%
                                                 # add plate covariate
                                                 merge(subset(lsarp.mpx.map.plates, standard.name %in% lsarp.lmer.kegg.pca.df$standard.name),
                                                       by="standard.name") %>%
                                                 tibble::column_to_rownames(var = "standard.name") %>%
...
lsarp.lmer.kegg.pca.plot <- ggplot(
  ...
  annotate(geom="text", x=0, y=Inf, hjust=0.5, vjust=1.5,
           label=paste(paste("R²: ", round(data.frame(lsarp.lmer.kegg.pca.permanova)[1,3], 3)*100, "%",
                             "  𝘱: ", round(data.frame(lsarp.lmer.kegg.pca.permanova)[1,5], 3), sep=""), sep=""))+
...
set.seed(25)
t1 = Sys.time()
lsarp.lmer.cog.pca.permanova = vegan::adonis2(dist(lsarp.lmer.cog.pca$x[,c(1,2)]) ~ flare.group + plate,
...
lsarp.lmer.cog.pca.plot <- ggplot(
  ...
  annotate(geom="text", x=0, y=Inf, hjust=0.5, vjust=1.5,
           label=paste(paste("R²: ", round(data.frame(lsarp.lmer.cog.pca.permanova)[1,3], 3)*100, "%",
                             "  𝘱: ", round(data.frame(lsarp.lmer.cog.pca.permanova)[1,5], 3), sep=""), sep=""))+
...
set.seed(25)
t1 = Sys.time()
lsarp.lmer.cazy.pca.permanova = vegan::adonis2(dist(lsarp.lmer.cazy.pca$x) ~ flare.group + plate,
...
lsarp.lmer.cazy.pca.plot <- ggplot(
  ...
  annotate(geom="text", x=0, y=Inf, hjust=0.5, vjust=1.5,
           label=paste(paste("R²: ", round(data.frame(lsarp.lmer.cazy.pca.permanova)[1,3], 3)*100, "%",
                             "  𝘱: ", round(data.frame(lsarp.lmer.cazy.pca.permanova)[1,5], 3), sep=""), sep=""))+
...
set.seed(25)
t1 = Sys.time()
lsarp.lmer.mbx.pca.permanova = vegan::adonis2(dist(lsarp.lmer.mbx.pca$x[,c(1,2)]) ~ flare.group,
...
lsarp.lmer.mbx.pca.plot <- ggplot(
  ...
  annotate(geom="text", x=0, y=Inf, hjust=0.5, vjust=1.5,
           label=paste(paste("R²: ", round(data.frame(lsarp.lmer.mbx.pca.permanova)[1,3], 3)*100, "%",
                             "  𝘱: ", round(data.frame(lsarp.lmer.mbx.pca.permanova)[1,5], 3), sep=""), sep=""))+
...
lsarp.rf.feature.change.with.rs = rbind(
  ...
  as.data.frame() %>%
  subset(group == "RS") %>%
  subset(taxa %in% c(lsarp.rf.models.importances.ttest.p$feature, "(s) Faecalibacterium_duncaniae")) %>%
  arrange(padj)
...
lsarp.rf.feature.change.with.rs = lsarp.rf.feature.change.with.rs %>%
  mutate(data.type = ifelse(taxa %in% c(colnames(lsarp.asv.baseline), "(s) Faecalibacterium_duncaniae"), "ASV",
                            ifelse(taxa %in% colnames(lsarp.mgx.baseline), "Species",
                                   ifelse(taxa %in% colnames(lsarp.kegg.baseline), "Pathway",
                                          ifelse(taxa %in% colnames(lsarp.cog.baseline), "COG",
                                                 ifelse(taxa %in% colnames(lsarp.cazy.baseline), "CAZyme", 
...
lsarp.rf.feature.change.with.rs.volcano = ggplot(lsarp.rf.feature.change.with.rs %>%
  ...
  ggrepel::geom_text_repel(aes(label=ifelse(padj < 0.20, gsub("\\..*", "", taxa), NA)),
                           size=2.5)+
  facet_wrap(~"Interaction 𝘱 value (Responders vs Non-Responders)")+
...
lsarp.ml.predictors.over.time.plot = ggplot(lsarp.ml.predictors.over.time%>%
  ...
  geom_point(data = lsarp.ml.predictors.over.time%>%
               ...
               subset(HM %in% substr(lsarp.omics.baseline$standard.name, 1, 6)) %>%
               #subset(lsarp.days < 0) %>%
               # remove missing data (e.g. 0 counts)
               dplyr::group_by(variable) %>% dplyr::filter(value > min(value)) %>%
               mutate(variable = ifelse(as.character(variable) == "g__Faecalibacterium_s__duncaniae",
                                        "(s) Faecalibacterium_duncaniae", as.character(variable))),
             shape=21, size=2.5, color="white",
             aes(fill = flare.group, x = lsarp.days, y=value))+
  # add lm p value
  geom_text(
    data = lsarp.rf.feature.change.with.rs %>%
      subset(taxa %in% c(as.character(lsarp.ml.predictors.over.time$variable), "(s) Faecalibacterium_duncaniae")) %>%
      mutate(variable = taxa) %>%
      mutate(variable = ifelse(as.character(variable) == "g__Faecalibacterium_s__duncaniae",
                               "(s) Faecalibacterium_duncaniae", as.character(variable))),
    aes(y=Inf, label = paste("𝘱:", round(pval, digits=3))), 
  ...
  geom_text(
    ...
    aes(y=Inf, label = paste("𝘱:", round(wilcox.pval, digits=3))), 
...
# are responders/non-responders on systematically different plates

lsarp.metadata.responders %>%
  mutate(plate = as.factor(plate)) %>%
  subset(lsarp.on.rs == "pre.rs") %>%
  dplyr::select(HM, flare.group, group, plate) %>%
  ggplot(aes(x=flare.group, y=plate))+
  geom_jitter(width=0.15, height=0.15,
              shape=21, aes(fill=flare.group), color="white", size=3)+
  facet_wrap(~group)+
  theme_classic()
# perfectly confounds placebo resp/non-resp
# but seems fairly matched for RS resp/non-resp
...
lsarp.metadata.responders %>%
  ...
  dplyr::select(flare.group, plate) %>%
  table() %>% fisher.test()
# not sig; p = 0.1061
lsarp.metadata.responders %>%
  subset(lsarp.on.rs == "pre.rs") %>%
...
metadata.lsarp.phasdial.resp.plot = ggplot(lsarp.metadata.responders.asv.phasdial %>%
  ...
  geom_text(data=pval.positioner("phasdial",
                                 lsarp.metadata.responders.asv.phasdial,
                                 data1=lsarp.resp.stats.phasdial.placebo,
                                 data2=lsarp.resp.stats.phasdial.treatment,
                                 type="response",
                                 yadjust=1.1), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
metadata.lsarp.mgx.phasdial.resp.plot = ggplot(lsarp.metadata.responders.mgx.phasdial %>%
  ...
  labs(x="Days since starting Product", y="Phascolarctobacterium/Dialister (Metagenomics)")+
       geom_text(data=pval.positioner("phasdial",
                                      lsarp.metadata.responders.mgx.phasdial,
                                      data1=lsarp.resp.stats.mgx.phasdial.placebo,
                                      data2=lsarp.resp.stats.mgx.phasdial.treatment,
                                      type="response",
                                      yadjust=1.05), 
                 aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
# stats
lsarp.resp.stats.mpx.succinate.treatment = lmerTest::lmer(scale((succ)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                         subset(lsarp.metadata.responders.mpx.succinate, group %in% c("RS")) %>%
                                                           subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.mpx.succinate.placebo = lmerTest::lmer(scale(succ) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
...
metadata.lsarp.mpx.succinate.resp.plot = ggplot(lsarp.metadata.responders.mpx.succinate %>%
  ...
  labs(x="Days since starting Product", y="Sum(Succinate-related COGs)")+
       geom_text(data=pval.positioner("succ",
                                      lsarp.metadata.responders.mpx.succinate,
                                      data1=lsarp.resp.stats.mpx.succinate.placebo,
                                      data2=lsarp.resp.stats.mpx.succinate.treatment,
                                      type="response",
                                      yadjust=1.05), 
                 aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
...
(metadata.lsarp.phasdial.resp.plot+
    metadata.lsarp.mgx.phasdial.resp.plot+
    metadata.lsarp.mpx.succinate.resp.plot) %>%
  ggsave(filename="./lsarp_plots/2026_03_01_lsarp_supp_phasco_invivo.pdf",
         width=20, height=4,device = cairo_pdf)
...
lsarp.px = read.csv("~/Downloads/results.pg_matrix.tsv", sep="\t")

# :: Unsupervised ---------------------------------------------------------

lsarp.px.all = lsarp.px

colnames(lsarp.px.all) = gsub("E..searches.Peter_rawstool_human.dia.", "", colnames(lsarp.px.all))
colnames(lsarp.px.all) = gsub("\\.raw.dia", "", colnames(lsarp.px.all))
colnames(lsarp.px.all) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_StintziLab_", "", colnames(lsarp.px.all))
colnames(lsarp.px.all) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_Stintzi_", "", colnames(lsarp.px.all))
lsarp.px.all = lsarp.px.all[,!grepl("QC", colnames(lsarp.px.all))]
colnames(lsarp.px.all) = gsub("\\_", "-", colnames((lsarp.px.all)))

# collapse to Protein.Name

# replace unannotated Genes with "unannotated"
lsarp.px.all$Protein.Names = ifelse(lsarp.px.all$Protein.Names == "", "unannotated", lsarp.px.all$Protein.Names)

# sum up protein intensities of the same Protein.Name name
lsarp.px.all.df = reshape2::melt(lsarp.px.all[,-c(1,3:6)])
lsarp.px.all.df = lsarp.px.all.df %>%
...
lsarp.px.all.df = reshape2::acast(lsarp.px.all.df, variable ~ Protein.Names, value.var="value")
dim(lsarp.px.all.df)

# replace NA with 0
lsarp.px.all.df[is.na(lsarp.px.all.df)] <- 0

# filter + process (no log)

# remove non-compliant
lsarp.px.all.mat.1 = lsarp.px.all.df[rownames(lsarp.px.all.df) %in% metadata.lsarp.stool$standard.name,] %>% as.data.frame()

# filter to 20% prevalence
dim(lsarp.px.all.mat.1) # 4484 proteins, 183 samples
lsarp.px.all.mat.filt.1 = lsarp.px.all.mat.1
lsarp.px.all.mat.filt.1[is.na(lsarp.px.all.mat.filt.1)] = 0
lsarp.px.all.mat.filt.1[lsarp.px.all.mat.filt.1!=0] = 1
lsarp.px.all.mat.filt.1 = lsarp.px.all.mat.1[,colSums(lsarp.px.all.mat.filt.1) >= nrow(lsarp.px.all.mat.filt.1)*0.2]
dim(lsarp.px.all.mat.filt.1) # 921 proteins, 183 samples


# :: PCA ------------------------------------------------------------------

# log transform with pseudo
lsarp.px.all.mat.filt.1 = log2(lsarp.px.all.mat.filt.1 + min(lsarp.px.all.mat.filt.1[lsarp.px.all.mat.filt.1!=0])/2)
# reduce to samples with metadata
lsarp.px.all.mat.filt.1 = lsarp.px.all.mat.filt.1[rownames(lsarp.px.all.mat.filt.1) %in% metadata.lsarp.stool$standard.name,]
# PCA
lsarp.host.mpx.pca = prcomp((lsarp.px.all.mat.filt.1), scale=T)
lsarp.host.mpx.pca.df = lsarp.host.mpx.pca$x[,c(1,2)] %>% data.frame() 
lsarp.host.mpx.pca.df$standard.name = rownames(lsarp.px.all.mat.filt.1)
lsarp.host.mpx.pca.df = merge(lsarp.host.mpx.pca.df,
                             metadata.lsarp.stool, by="standard.name")
lsarp.host.mpx.pca.var <- (lsarp.host.mpx.pca$sdev^2 / sum(lsarp.host.mpx.pca$sdev^2)) * 100

rownames(lsarp.host.mpx.pca.df) = lsarp.host.mpx.pca.df$standard.name

if(sum(colnames(lsarp.host.mpx.pca.df) %in% "plate")==0){
  lsarp.host.mpx.pca.df = merge(lsarp.host.mpx.pca.df, 
                                lsarp.mpx.map.plates, by="standard.name")
  lsarp.host.mpx.pca.df$plate = as.factor(lsarp.host.mpx.pca.df$plate)
}

# permanova
set.seed(25)
t1 = Sys.time()
lsarp.host.mpx.pca.permanova = vegan::adonis2(dist(lsarp.host.mpx.pca$x) ~ group*on.rs + ave.fiber+plate,
                                              lsarp.host.mpx.pca.df %>%
                                                  mutate(on.rs = as.factor(ifelse(lsarp.on.rs=="on.rs", "on.rs", "off.rs"))),
                                                strata = lsarp.host.mpx.pca.df$HM,
                                                by="margin")
t2 = Sys.time()
t2 - t1
lsarp.host.mpx.pca.permanova
# sig

lsarp.host.mpx.pca.plot <-ggplot(
  data=lsarp.host.mpx.pca.df %>% group_by(HM) %>% arrange(stool_date_rec_v2) %>%
   # mutate(RS_Name = factor(RS_Name, levels=rs.names)) %>%
    mutate(Group = group), 
  aes(x=PC1, y=PC2))+
  geom_path(aes(group=HM), color="black", linetype=2, alpha=0.5, linewidth=0.3) + 
  ...
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x=paste("PC 1: ", round((lsarp.host.mpx.pca.var)[1], digits=2), "%", sep=""), 
       y=paste("PC 2: ", round((lsarp.host.mpx.pca.var)[2], digits=2), "%", sep=""),
       title=paste(paste("Host Protein\nTreatment R²: ", round(data.frame(lsarp.host.mpx.pca.permanova)[3,3], 3)*100, "%",
                         "  𝘱: ", round(data.frame(lsarp.host.mpx.pca.permanova)[3,5], 3), sep="")), sep="")
lsarp.host.mpx.pca.plot


# :: lmer ----------------------------------------------------

lsarp.lmer.mpx.host.data = lsarp.delta.omic.prepare(lsarp.px.all.mat.1[rownames(lsarp.px.all.mat.1) %in% stools.not.flare,],
                                                    normalize=F)
dim(lsarp.lmer.mpx.host.data)

lsarp.lmer.mpx.host.data = lsarp.lmer.mpx.host.data[,colnames(lsarp.lmer.mpx.host.data) != "X."]

lsarp.lmer.mpx.host.interactions = lsarp.delta.omic.lmer(lsarp.lmer.mpx.host.data,
                                                         split=F,
                                                         mpx = T) 

lsarp.lmer.mpx.host.interactions.plot = ggplot(lsarp.lmer.mpx.host.interactions%>%
                                                 mutate(phase = ifelse(phase == "treatment", "Treatment", "Washout")),
                                               aes(x=estimate, y=padj))+
  geom_point(shape=21, aes(fill=estimate))+
  geom_hline(yintercept=(0.20), linetype=2, alpha=0.5)+
  ...
  ggnetwork::geom_nodetext_repel(aes(label=ifelse(pval < 0.05 & padj > 0.2, taxa, NA)),
                                 size=2.5, color="grey")+
  labs(x="Interaction Coefficient",
       title="Group Interaction (Host Protein)",
       y="FDR")+
  facet_wrap(~phase)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        ...
                        strip.background = element_rect(
                          color="black"))
lsarp.lmer.mpx.host.interactions.plot

subset(lsarp.lmer.mpx.host.interactions, padj < 0.2 & phase == "treatment" & estimate < 0) %>% nrow()
subset(lsarp.lmer.mpx.host.interactions, padj < 0.2 & phase == "washout") %>% nrow()

subset(lsarp.lmer.mpx.host.interactions, padj < 0.2 & phase == "treatment")
# treatment - decreased: 
...
# treatment - increased:
# LYSC = serine protease
# ANX11 = annexin A11; cytokinesis, binds calcyclin (S100A6)
# ITLN2 = Intelectin-2; defense against pathogens; STRING == LTF and MUC5AC
# AMYP = pancreatic amylase
# STOM = stomatin; lipid rafts

subset(lsarp.lmer.mpx.host.interactions, padj < 0.2 & phase == "washout")
# washout - increased
# LMAN2 = lectin, mannose binding 2; ER-associated
# CY1 = cytochrome C1 (mitochondrial)
# COX5A = cytochrome C oxidase (mitochondrial)
# SSRA = ER-associated
# IMDH2 = inosine-5'-monophosphate dehydrogenase; guanine synthesis
# FCAR = immunoglobulin alpha Fc receptor; cytokine signaling
# HSP7C = heat shock protein
# SC11A = ER membrane
# TMED7 = ER
# NAMPT = NAD+ salvage
# CTNA1 = cadherin; structural
# TM9S3 = Golgi
# ETFA = electron transfer; mitochondrial **
# SDHA = succinate dehydrogenase; mitochondria, TCA
# GNA13 = guanine binding
# ATPO = mitochondrial ***
# SUCB2 = succinate-CoA ligase; TCA cycle ***
# RAB5B = transport vesicles
# CTND1 = catenin; structural
# PCYOX = ER; lipid metabolism
# H0Y4K8 = fibronectin; cell structure
# BGAL = beta-galactosidase
# ITB1 = integrin beta-1; cell adhesion
# IDHC = isocitrate dehydrogenase; mitochondrial; TAC
# GPX3 = glutathione peroxidase; antioxidant
# PCBP1 = microRNA regulator
# ETFB = electron transfer ***
# RL12 = ribosome
# TBB3 = tubulin; cytoskeleton ***
# SSBP = mitochondrial replication
# PTER = N-acetyltaurine hydrolase
# PRDX6 = peroxiredoxin; redox balance
# ACOX1 = acyl-CoA oxidase; very LCFA oxidation ***
# MGST1 = redox
# PSB2 = proteosome
# MMP9 
...
# RL31 = ribosomal
# FABP5 = fatty acid binding ***
# ACSL5 = lipid metabolism; mitochondrial/ER
# CLH1 = clathrin, heavy chain
# SAMP = precursor to amyloid P
# SYPL1 = intracellular trafficking; biomarker for CRC
# PDIA4 = ER stress
# TXTP = mitochondrial
# OST48 = ER protein
# GRAN = growth factor + inflammation regulator



# Washout - decreased:
# A4 = amyloid-beta precursor
# TRY2 = serine protease
# 
...
lsarp.lmer.host.mpx.resp.interactions = lsarp.delta.omic.resp.lmer(lsarp.lmer.mpx.host.data[rownames(lsarp.lmer.mpx.host.data) %in% stools.not.flare,],
                                                                  split=F,
                                                                  mpx = T)
lsarp.lmer.host.mpx.resp.interactions.scaled = lsarp.delta.omic.resp.lmer(lsarp.lmer.mpx.host.data[rownames(lsarp.lmer.mpx.host.data) %in% stools.not.flare,],
                                                                   split=F,
                                                                   scale=T,
                                                                   mpx = T)

lsarp.lmer.host.mpx.resp.interactions.plot = ggplot(lsarp.lmer.host.mpx.resp.interactions,
                                                   aes(x=estimate, y=padj))+
  geom_point(shape=21, aes(fill=estimate))+
  geom_hline(yintercept=(0.20), linetype=2, alpha=0.5)+
  scale_fill_gradient2(low="blue", high="red")+
  ...
  ggrepel::geom_text_repel(aes(label=ifelse(pval < 0.05 & padj >0.2, taxa, NA)),
                           size=2.5, color="grey")+
  labs(x="Interaction Coefficient",
       title="Response Interaction (Host Protein)",
       y="FDR")+
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        ...
                        strip.background = element_rect(
                          color="black"))
lsarp.lmer.host.mpx.resp.interactions.plot

subset(lsarp.lmer.host.mpx.resp.interactions, taxa == "S10A8")
# S100A8 is reduced in RS responders (p = 0.007) but not non-responders (p = 0.81)
subset(lsarp.lmer.host.mpx.resp.interactions, taxa == "S10A9")
# S100A9 is reduced in RS responders (p = 0.03) but not non-responders (p = 0.59)
subset(lsarp.lmer.host.mpx.resp.interactions, taxa == "NDUAD")
# NDUAD is increased in RS responders (p = 0.01) but not non-responders (reduced, p = 0.087)
subset(lsarp.lmer.host.mpx.resp.interactions, taxa == "TRFL")
# TRFL is reduced in RS responders (p = 0.02) but not non-responders (p = 0.75)

subset(lsarp.lmer.host.mpx.resp.interactions, group == "RS" & padj < 0.25)
subset(lsarp.lmer.host.mpx.resp.interactions.scaled, group == "RS" & padj < 0.25)

# RS - increased in responders
# THTR = thiamine transporter

# RS - decreased in responders
# STOM = stomatin; lipid rafts (increased in RS at group-level)
# CAP7 = cationic antimicrobial protein; neutrophil granule protein

subset(lsarp.lmer.host.mpx.resp.interactions, group == "Placebo" & padj < 0.25)
# Placebo - increased in responders
# S10A6 = calcium binding protein
# CO1A2 = collagen
# PRPC = protein-serine/threonine phosphatase
# H37 = histone

# Placebo - decreased in responders
# TPM3 = tropomyosin; structural
# TNNI1 = troponin; structural
# MLRV = myosin; structural
# COX6C = cytochrome C oxidase
# CRYAB = crystallin B; amyloid-beta binding

# is thiamine production associated with THTR?
subset(lsarp.lmer.mpx.cog.resp.interactions, group == "RS" & padj < 0.25)
subset(lsarp.lmer.mpx.cog.resp.interactions, group == "RS" & grepl("hiamin", taxa) & grepl("biosyn", taxa))
subset(lsarp.lmer.mpx.cog.resp.interactions, group == "RS" & grepl("hiamin", taxa) & grepl("salva", taxa))

subset(lsarp.lmer.mpx.cog.resp.interactions.scaled, group == "RS" & grepl("hiamin", taxa) & grepl("biosyn", taxa))
subset(lsarp.lmer.mpx.cog.resp.interactions.scaled, group == "RS" & grepl("hiamin", taxa) & grepl("salva", taxa))

# :: S100A89  -----------------------------------------------------


lsarp.px$Protein.Names[grepl("S10A9", lsarp.px$Protein.Names)]
which(lsarp.px$Protein.Names == "S10A8_HUMAN") # 936
which(lsarp.px$Protein.Names == "S10A9_HUMAN") # 983

lsarp.px.clean = lsarp.px
colnames(lsarp.px.clean) = gsub("E..searches.Peter_rawstool_human.dia.", "", colnames(lsarp.px.clean))
colnames(lsarp.px.clean) = gsub("\\.raw.dia", "", colnames(lsarp.px.clean))
colnames(lsarp.px.clean) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_StintziLab_", "", colnames(lsarp.px.clean))
colnames(lsarp.px.clean) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_Stintzi_", "", colnames(lsarp.px.clean))
lsarp.px.clean = lsarp.px.clean[,!grepl("QC", colnames(lsarp.px.clean))]
colnames(lsarp.px.clean) = gsub("\\_", "-", colnames((lsarp.px.clean)))

lsarp.px.s100a89 = lsarp.px.clean[grepl((paste(c("S10A8", "S10A9"), collapse="|")), lsarp.px.clean$Protein.Names),-c(1:6)]

# check
plot(as.numeric(lsarp.px.s100a89[1,]),
     as.numeric(lsarp.px.s100a89[2,]))
cor.test(as.numeric(lsarp.px.s100a89[1,]),
         as.numeric(lsarp.px.s100a89[2,]), method="spearman")
# correlates well; R = 0.93, p < 0.0001

lsarp.s100a89 = data.frame(standard.name = colnames(lsarp.px.s100a89),
                          s100a8 = as.numeric(lsarp.px.s100a89[1,]),
                          s100a9 = as.numeric(lsarp.px.s100a89[2,]))

# clean up sample names
lsarp.s100a89$standard.name = gsub("\\.1", "", lsarp.s100a89$standard.name)
lsarp.s100a89 = subset(lsarp.s100a89, standard.name != "HM0926-01-A")
# good

# combine A8+A9 (and take average)
lsarp.s100a89 = lsarp.s100a89 %>%
...
lsarp.s100a89.all = merge(lsarp.s100a89,
                           metadata.lsarp.stool, by="standard.name", all.x=F)
lsarp.s100a89.all$phase = ifelse(lsarp.s100a89.all$phase == "treatment", "Treatment", "Washout")

# and correlation plot
lsarp.s100a89.response.cor.plot = ggplot(lsarp.s100a89.all %>%
                                           subset(flare != "flare"& !HM %in% excluded.by.dave),
                                         aes(x=fcal, 
                                             y=s100a89))+
  scale_y_log10()+
  ...
  scale_shape_manual(values=c(23,21))+
  #scale_fill_manual(values=c("grey",labelcolors$cols[c(1,1,4,5,5,8,9)]))+
  labs(x="Fecal Calprotectin (μg/g)", y="S100A8/A9")+
  ggpubr::stat_cor( method="spearman")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text = element_text(size=10),
                        strip.background = element_rect(
                          color="black"))
lsarp.s100a89.response.cor.plot
# 0.71 to 0.75; 0.72 overall

lsarp.stats.s100a89.treatment = lmerTest::lmer(scale(log10(s100a89)) ~ group*scale(lsarp.days) + ave.fiber + (1|HM) + (1|plate),
                                            subset(lsarp.s100a89.all, 
                                                   phase %in% c("Treatment"))%>%
                                              # remove samples collected after flare
                                              subset(flare != "flare" & !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.stats.s100a89.washout = lmerTest::lmer(scale(log10(s100a89)) ~ group*scale(lsarp.days) + ave.fiber +  (1|HM)+ (1|plate),
                                          subset(lsarp.s100a89.all, 
                                                 phase == "Washout" | rs.end == "rs.end") %>%
                                            # remove samples collected after flare
                                            subset(flare != "flare") %>%
...
# plot
metadata.lsarp.s100a89.plot = ggplot(lsarp.s100a89.all %>%
                                    subset(!is.na(fcal))%>%
                                    # remove samples collected after flare
                                    subset(flare != "flare"& !HM %in% excluded.by.dave),
                                  aes(x=lsarp.days, y=s100a89))+
  scale_y_log10()+
  annotate("rect", xmin=0, xmax=max(subset(lsarp.s100a89.all, 
                                           phase=="Treatment")$lsarp.days),
           ymin=0, ymax=Inf, alpha=0.2, fill="salmon") +
  geom_line(aes(group=HM), linetype=2, alpha=0.5, linewidth=0.3)+
  geom_smooth(color="white", span=1)+
  ...
  scale_alpha_manual(values=c(0.5, 1))+
  scale_fill_manual(values=rs.colors, na.value = "grey")+
  labs(x="Days since starting Product", y="S100A8/A9")+
  geom_text(data=pval.positioner("s100a89",
                                 lsarp.s100a89.all,
                                 lsarp.stats.s100a89.treatment,
                                 lsarp.stats.s100a89.washout,
                                 type="group",
                                 yadjust=1.05), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group)+
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))
metadata.lsarp.s100a89.plot


# check remitters vs relapsers

# now compare to calprotectin
lsarp.s100a89.resp = merge(lsarp.s100a89,
                      lsarp.metadata.responders, by="standard.name", all.x=F)

# how many samples lack MPX?
sum(lsarp.s100a89$standard.name %in% metadata.lsarp.stool$standard.name)
# 180

range(lsarp.s100a89$s100a8)
range(lsarp.s100a89$s100a9)

plot(log10(lsarp.s100a89$fcal),
     lsarp.s100a89$s100a89)
cor.test(log10(lsarp.s100a89$fcal),
         lsarp.s100a89$s100a89, method="spearman")
# R = 0.59, p < 0.0001

# stats
lsarp.resp.stats.s100a89.treatment = lmerTest::lmer(scale(log10(s100a89)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                   subset(lsarp.s100a89.resp, group %in% c("RS")) %>%
                                                     subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.s100a89.placebo = lmerTest::lmer(scale(log10(s100a89)) ~ scale(lsarp.days)*flare.group + ave.fiber + (1|HM) + (1|plate),
                                                 subset(lsarp.s100a89.resp, group %in% c("Placebo")) %>%
                                                   subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

# plot
metadata.lsarp.s100a89.resp.plot = ggplot(lsarp.s100a89.resp %>%
                                           subset(flare != "flare"& !HM %in% excluded.by.dave),
                                         aes(x=lsarp.days, 
                                             y=s100a89))+
  scale_y_log10()+
  ...
  labs(x="Days since starting Product", y="S100A8/A9")+
  geom_text(data=pval.positioner("s100a89",
                                 lsarp.s100a89.resp %>% mutate(phase = "treatment"),
                                 data1=lsarp.resp.stats.s100a89.placebo,
                                 data2=lsarp.resp.stats.s100a89.treatment,
                                 type="response",
                                 yadjust=1.05), 
            aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  facet_wrap(~group,scales="free_x")+
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))
metadata.lsarp.s100a89.resp.plot


# :: Enrichment -----------------------------------------------------------

lsarp.lmer.mpx.host.interactions

lsarp.lmer.host.mpx.resp.interactions

# use biomaRt to assign GO 

sum(lsarp.lmer.mpx.host.interactions$taxa %in% lsarp.lmer.host.mpx.resp.interactions$taxa)
sum(lsarp.lmer.host.mpx.resp.interactions$taxa %in% lsarp.lmer.mpx.host.interactions$taxa)
# same

lsarp.mpx.host.proteins = lsarp.lmer.mpx.host.interactions$taxa
# delimit
lsarp.mpx.host.proteins = unlist(strsplit(lsarp.mpx.host.proteins, ";"))
...
# recall that these have the same list of protein names
lsarp.lmer.mpx.host.interactions
lsarp.lmer.host.mpx.resp.interactions

# goal: use UniProtDB to do conversions

write.csv(data.frame(id = c(unlist(strsplit(lsarp.lmer.mpx.host.interactions$taxa, ";")),
                            paste(unlist(strsplit(lsarp.lmer.mpx.host.interactions$taxa, ";")), "_HUMAN", sep=""))),
          "./host_protein_name.csv")
# https://www.uniprot.org/id-mapping
lsarp.mpx.protein.genes = read.csv("./idmapping_2026_07_26.tsv", sep="\t")

# now create df's with gene name appended
# take first gene entry
lsarp.mpx.protein.genes$gene = gsub(" .*", "", lsarp.mpx.protein.genes$Gene.Names)
lsarp.mpx.protein.genes$protein = gsub("_HUMAN", "", lsarp.mpx.protein.genes$From)

lsarp.mpx.protein.genes = lsarp.mpx.protein.genes[,c("gene", "protein")]

# append to data
lsarp.lmer.mpx.host.interactions$gene = lsarp.mpx.protein.genes$gene[match(lsarp.lmer.mpx.host.interactions$taxa, lsarp.mpx.protein.genes$protein)]
lsarp.lmer.host.mpx.resp.interactions$gene = lsarp.mpx.protein.genes$gene[match(lsarp.lmer.host.mpx.resp.interactions$taxa, lsarp.mpx.protein.genes$protein)]

host_pro_go_map_alias_gene <- bitr(
  geneID   = unlist(strsplit(lsarp.lmer.mpx.host.interactions$gene, ";")),
  fromType = "ALIAS",
  toType   = c("GO", "ONTOLOGY", "SYMBOL", "GENENAME"),
  OrgDb    = org.Hs.eg.db
)
...
# run enrichment
t1 = Sys.time()
go_results_treatment_fdr_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, padj < 0.20 & phase == "treatment")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.mpx.host.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
  pAdjustMethod = "BH",
  pvalueCutoff  = 1,
  qvalueCutoff  = 1,
  readable      = TRUE            # gives gene symbols too
)
t2 = Sys.time()
t2 - t1 # 5 min
head(summary(go_results_treatment_fdr_gene))
# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, padj < 0.20 & phase == "treatment")$gene, ";")) %in% unique(host_pro_go_map_alias_gene$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, padj < 0.20 & phase == "treatment")$gene, ";")))
# 100%

t1 = Sys.time()
go_results_treatment_p_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, pval < 0.05 & phase == "treatment")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.mpx.host.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
...
t2 = Sys.time()
t2 - t1 # 15 min
head(summary(go_results_treatment_p_gene))
# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, pval < 0.05 & phase == "treatment")$gene, ";")) %in% unique(host_pro_go_map_alias_gene$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, pval < 0.05 & phase == "treatment")$gene, ";")))
# 96% NICE
...
# run enrichment
t1 = Sys.time()
go_results_washout_fdr_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, padj < 0.20 & phase == "washout")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.mpx.host.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
...
t2 = Sys.time()
t2 - t1 # 5 min
head(summary(go_results_washout_fdr_gene))
# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, padj < 0.20 & phase == "washout")$gene, ";")) %in% unique(host_pro_go_map_alias_gene$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, padj < 0.20 & phase == "washout")$gene, ";")))
# 100%

t1 = Sys.time()
go_results_washout_p_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, pval < 0.05 & phase == "washout")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.mpx.host.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
...
t2 = Sys.time()
t2 - t1 # 5 min
head(summary(go_results_washout_p_gene))
# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, pval < 0.05 & phase == "washout")$gene, ";")) %in% unique(host_pro_go_map_alias_gene$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.mpx.host.interactions, pval < 0.05 & phase == "washout")$gene, ";")))
# 99%
...
# run enrichment
t1 = Sys.time()
go_results_rs_fdr_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, padj < 0.20 & group == "RS")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.host.mpx.resp.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
  pAdjustMethod = "BH",
...
t2 = Sys.time()
t2 - t1 # 5 min
head(summary(go_results_rs_fdr_gene))
# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, padj < 0.20 & group == "RS")$gene, ";")) %in% unique(host_pro_go_map_alias$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, padj < 0.20 & group == "RS")$gene, ";")))
# NA - none were sig

t1 = Sys.time()
go_results_rs_p_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, pval < 0.05 & group == "RS")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.host.mpx.resp.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
...
t2 = Sys.time()
t2 - t1 # 15 min
head(summary(go_results_rs_p_gene))
# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, pval < 0.05 & group == "RS")$gene, ";")) %in% unique(host_pro_go_map_alias_gene$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, pval < 0.05 & group == "RS")$gene, ";")))
# 100%
...
# run enrichment
t1 = Sys.time()
go_results_placebo_fdr_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, padj < 0.20 & group == "Placebo")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.host.mpx.resp.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
...
t2 = Sys.time()
t2 - t1 # 5 min
head(summary(go_results_placebo_fdr_gene))
# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, padj < 0.20 & group == "Placebo")$gene, ";")) %in% unique(host_pro_go_map_alias_gene$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, padj < 0.20 & group == "Placebo")$gene, ";")))
# 100%

t1 = Sys.time()
go_results_placebo_p_gene <- enrichGO(
  gene          = unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, pval < 0.05 & group == "Placebo")$gene, ";")), # sig proteins
  universe      = unlist(strsplit(lsarp.lmer.host.mpx.resp.interactions$gene, ";")),   # all background proteins
  OrgDb         = org.Hs.eg.db,
  keyType       = "ALIAS",      # Important!
  ont           = "BP",          # BP + MF + CC
...
t2 = Sys.time()
t2 - t1 # 15 min
head(summary(go_results_placebo_p_gene))

# but how many proteins are actually being analyzed? is this a viable analysis?
sum(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, pval < 0.05 & group == "Placebo")$gene, ";")) %in% unique(host_pro_go_map_alias_gene$ALIAS)) / 
  length(unlist(strsplit(subset(lsarp.lmer.host.mpx.resp.interactions, pval < 0.05 & group == "Placebo")$gene, ";")))
# 100%

# Pair with volcano plot
lsarp.lmer.mpx.host.interactions.plot

host.pro.enrich.plot.1 = dotplot(go_results_treatment_p_gene, showCategory = 10,
        color = "p.adjust") + 
  ggtitle("Treatment") +
  theme_bw()

host.pro.enrich.plot.2 = dotplot(go_results_washout_p_gene, showCategory = 10,
        color = "p.adjust") + 
  ggtitle("Washout") +
  theme_bw()
...
host.pro.enrich.plot.3 = dotplot(go_results_rs_p_gene, showCategory = 10,
        color = "p.adjust") + 
  ggtitle("Placebo") +
  theme_bw()

host.pro.enrich.plot.4 = dotplot(go_results_placebo_p_gene, showCategory = 10,
        color = "p.adjust") + 
  ggtitle("RS") +
  theme_bw()


cowplot::plot_grid(lsarp.lmer.host.mpx.resp.interactions.plot,
    patchwork::free(host.pro.enrich.plot.3 + host.pro.enrich.plot.4 + patchwork::plot_layout(nrow=1), type="label"),
    nrow=2, rel_heights=c(1,1)) %>%
  ggsave(filename="./lsarp_plots/2026_07_28_stool_host_proteins_resp.pdf",
...
cowplot::plot_grid((lsarp.host.mpx.pca.plot/lsarp.lmer.mpx.host.interactions.plot),
   patchwork::free(host.pro.enrich.plot.1 + host.pro.enrich.plot.2 + patchwork::plot_layout(nrow=1), type="label"),
   nrow=2, rel_heights=c(2,1)) %>%
  ggsave(filename="./lsarp_plots/2026_07_28_host_proteomics_group.pdf",
         width=9, height=12, device = cairo_pdf)

(lsarp.s100a89.response.cor.plot + 
    metadata.lsarp.s100a89.resp.plot+
    patchwork::plot_layout(widths=c(1,2)))%>%
  ggsave(filename="./lsarp_plots/2026_03_01_fcal_s100a8a9.pdf",
...
# Mapping files
lsarp.omic.heatmap.mapping.1 = list(
  Group = c(`RS` = "#00BFC4", `Placebo` = "#F8766D"),
  ...
  `Days on product` = colorRampPalette(c("white","grey", "black"))(100)
)

lsarp.omic.heatmap.mapping.2 = list(
  Group = c(`RS` = "#00BFC4", `Placebo` = "#F8766D"),
  RS_Type = rs.colors.type,
  Coefficient = colorRampPalette(c("blue","white", "red"))(100),
  Fiber = colorRampPalette(c("blue","white", "red"))(100),
  Significance =  c(`𝑝 < 0.05` = "lightgrey",  `FDR < 0.20` = "lightblue", `FDR < 0.05` = "blue"),
  `Days off product` = colorRampPalette(c("white","grey", "black"))(100)
)
...
deid.fun = function(x) {
    # Split by comma (handles multiple entries)
    parts <- str_split(x, "\\s*,\\s*") %>% unlist()
    
    # Replace each part
    new_parts <- sapply(parts, function(p) {
      # Extract the HMxxx part (before first "-")
      hm_part <- str_extract(p, "^[^-]+")
      replace_map = setNames(lsarp.cd.deid$code, lsarp.cd.deid$HM)
      if (hm_part %in% names(replace_map)) {
        code <- replace_map[hm_part]
        str_replace(p, "^[^-]+", code)
      } else {
        p  # keep original if no match
      }
    })
...
# use like this
result <- sapply(your_vector, deid.fun)

# :: :: ASV ---------------------------------------------------------------

subset(lsarp.lmer.asv.interactions, pval <= 0.1)
lsarp.lmer.asv.data

lsarp.lmer.asv.data.hm = lsarp.lmer.asv.data


lsarp.asv.heatmap.data.treatment = (lsarp.lmer.asv.data[rownames(lsarp.lmer.asv.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment")$standard.name,
                                                      colnames(as.data.frame(lsarp.lmer.asv.data)) %in% 
                                                        subset(lsarp.lmer.asv.interactions,phase == "treatment" & pval <= 0.1)$taxa]) %>%
                                      log10() %>% as.matrix() %>% scale() %>% t() 

lsarp.asv.heatmap.data.washout = (lsarp.lmer.asv.data[rownames(lsarp.lmer.asv.data) %in%subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")$standard.name,
                                                    colnames(as.data.frame(lsarp.lmer.asv.data)) %in% 
                                                      subset(lsarp.lmer.asv.interactions, phase == "washout" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>%  # scaled per column
  t() 

pheatmap::pheatmap((lsarp.asv.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.asv.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.asv.heatmap.data.treatment)/100, max(lsarp.asv.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.asv.interactions,phase == "treatment" & pval <= 0.1)%>%
                     ...
                     dplyr::select(sig, estimate) %>%
                     mutate(Coefficient = estimate,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.lsarp.stool, phase == "treatment")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Group = group,
                            `Days on product` = lsarp.days,
                            Fiber = ave.fiber) %>% dplyr::select(Group, `Days on product`, Fiber, RS_Type),
                   annotation_colors = lsarp.omic.heatmap.mapping.1,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.asv.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(lsarp.asv.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "ASV | Treatment Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_asv_tr_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)


pheatmap::pheatmap((lsarp.asv.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.asv.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.asv.heatmap.data.washout)/100, max(lsarp.asv.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.asv.interactions,phase == "washout" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                     ...
                     mutate(Coefficient = estimate,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.lsarp.stool, phase == "washout" | rs.end == "rs.end")[,c("HM", "lsarp.off", "group", "RS_Name", "ave.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Group = group,
                            `Days off product` = lsarp.off,
                            Fiber = ave.fiber) %>% dplyr::select(Group, `Days off product`, Fiber, RS_Type),
                   annotation_colors = lsarp.omic.heatmap.mapping.2,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.asv.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(lsarp.asv.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "ASV | Washout Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_asv_wo_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)
...
lsarp.mgx.heatmap.data.treatment = (lsarp.lmer.mgx.data[rownames(lsarp.lmer.mgx.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment")$standard.name,
                                                        colnames(as.data.frame(lsarp.lmer.mgx.data)) %in% 
                                                          subset(lsarp.lmer.mgx.interactions,phase == "treatment" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

lsarp.mgx.heatmap.data.washout = (lsarp.lmer.mgx.data[rownames(lsarp.lmer.mgx.data) %in%subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")$standard.name,
                                                      colnames(as.data.frame(lsarp.lmer.mgx.data)) %in% 
                                                        subset(lsarp.lmer.mgx.interactions, phase == "washout" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>%  # scaled per column
  t() 

pheatmap::pheatmap((lsarp.mgx.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mgx.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mgx.heatmap.data.treatment)/100, max(lsarp.mgx.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mgx.interactions,phase == "treatment" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.1,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mgx.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(lsarp.mgx.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "Species | Treatment Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mgx_tr_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)


pheatmap::pheatmap((lsarp.mgx.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mgx.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mgx.heatmap.data.washout)/100, max(lsarp.mgx.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mgx.interactions,phase == "washout" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.2,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mgx.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(lsarp.mgx.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Species | Washout Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mgx_wo_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)

# :: :: KEGG ------------------------------------------------------------------

lsarp.mpx.kegg.heatmap.data.treatment = (lsarp.lmer.mpx.kegg.data[rownames(lsarp.lmer.mpx.kegg.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment")$standard.name,
                                                        colnames(as.data.frame(lsarp.lmer.mpx.kegg.data)) %in% 
                                                          subset(lsarp.lmer.mpx.kegg.interactions,phase == "treatment" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

lsarp.mpx.kegg.heatmap.data.washout = (lsarp.lmer.mpx.kegg.data[rownames(lsarp.lmer.mpx.kegg.data) %in%subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")$standard.name,
                                                      colnames(as.data.frame(lsarp.lmer.mpx.kegg.data)) %in% 
                                                        subset(lsarp.lmer.mpx.kegg.interactions, phase == "washout" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>%  # scaled per column
  t() 

pheatmap::pheatmap((lsarp.mpx.kegg.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.kegg.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.kegg.heatmap.data.treatment)/100, max(lsarp.mpx.kegg.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.kegg.interactions,phase == "treatment" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.1,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.kegg.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(lsarp.mpx.kegg.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "Pathway | Treatment Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mpx_kegg_tr_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)


pheatmap::pheatmap((lsarp.mpx.kegg.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.kegg.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.kegg.heatmap.data.washout)/100, max(lsarp.mpx.kegg.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.kegg.interactions,phase == "washout" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.2,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.kegg.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(lsarp.mpx.kegg.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Pathway | Washout Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mpx_kegg_wo_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)

# :: :: COG ------------------------------------------------------------------

lsarp.mpx.cog.heatmap.data.treatment = (lsarp.lmer.mpx.cog.data[rownames(lsarp.lmer.mpx.cog.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment")$standard.name,
                                                                  colnames(as.data.frame(lsarp.lmer.mpx.cog.data)) %in% 
                                                                    subset(lsarp.lmer.mpx.cog.interactions,phase == "treatment" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

lsarp.mpx.cog.heatmap.data.washout = (lsarp.lmer.mpx.cog.data[rownames(lsarp.lmer.mpx.cog.data) %in%subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")$standard.name,
                                                                colnames(as.data.frame(lsarp.lmer.mpx.cog.data)) %in% 
                                                                  subset(lsarp.lmer.mpx.cog.interactions, phase == "washout" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>%  # scaled per column
  t() 

pheatmap::pheatmap((lsarp.mpx.cog.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cog.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cog.heatmap.data.treatment)/100, max(lsarp.mpx.cog.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cog.interactions,phase == "treatment" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.1,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cog.heatmap.data.treatment))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "COG | Treatment Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mpx_cog_tr_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)


pheatmap::pheatmap((lsarp.mpx.cog.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cog.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cog.heatmap.data.washout)/100, max(lsarp.mpx.cog.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cog.interactions,phase == "washout" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.2,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cog.heatmap.data.washout))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "COG | Washout Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mpx_cog_wo_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)

# :: :: CAZy ------------------------------------------------------------------

lsarp.mpx.cazy.heatmap.data.treatment = (lsarp.lmer.mpx.cazy.data[rownames(lsarp.lmer.mpx.cazy.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment")$standard.name,
                                                                colnames(as.data.frame(lsarp.lmer.mpx.cazy.data)) %in% 
                                                                  subset(lsarp.lmer.mpx.cazy.interactions,phase == "treatment" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

lsarp.mpx.cazy.heatmap.data.washout = (lsarp.lmer.mpx.cazy.data[rownames(lsarp.lmer.mpx.cazy.data) %in%subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")$standard.name,
                                                              colnames(as.data.frame(lsarp.lmer.mpx.cazy.data)) %in% 
                                                                subset(lsarp.lmer.mpx.cazy.interactions, phase == "washout" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>%  # scaled per column
  t() 

pheatmap::pheatmap((lsarp.mpx.cazy.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cazy.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cazy.heatmap.data.treatment)/100, max(lsarp.mpx.cazy.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cazy.interactions,phase == "treatment" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.1,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cazy.heatmap.data.treatment))$new,
                   fontsize_row =10 - nrow(lsarp.mpx.cazy.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "CAZyme | Treatment Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mpx_cazy_tr_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)


pheatmap::pheatmap((lsarp.mpx.cazy.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cazy.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cazy.heatmap.data.washout)/100, max(lsarp.mpx.cazy.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cazy.interactions,phase == "washout" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.2,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cazy.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(lsarp.mpx.cazy.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "CAZyme | Washout Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mpx_cazy_wo_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)

# :: :: MBX ------------------------------------------------------------------

lsarp.mbx.heatmap.data.treatment = (lsarp.lmer.mbx.data[rownames(lsarp.lmer.mbx.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment")$standard.name,
                                                                  colnames(as.data.frame(lsarp.lmer.mbx.data)) %in% 
                                                                    subset(lsarp.lmer.mbx.interactions,phase == "treatment" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

lsarp.mbx.heatmap.data.washout = (lsarp.lmer.mbx.data[rownames(lsarp.lmer.mbx.data) %in%subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")$standard.name,
                                                                colnames(as.data.frame(lsarp.lmer.mbx.data)) %in% 
                                                                  subset(lsarp.lmer.mbx.interactions, phase == "washout" & pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>%  # scaled per column
  t() 

pheatmap::pheatmap((lsarp.mbx.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mbx.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mbx.heatmap.data.treatment)/100, max(lsarp.mbx.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mbx.interactions,phase == "treatment" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.1,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mbx.heatmap.data.treatment))$new,
                   fontsize_row =10 - nrow(lsarp.mbx.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Metabolite | Treatment Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mbx_tr_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)


pheatmap::pheatmap((lsarp.mbx.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mbx.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mbx.heatmap.data.washout)/100, max(lsarp.mbx.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mbx.interactions,phase == "washout" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.2,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mbx.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(lsarp.mbx.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Metabolite | Washout Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_mbx_wo_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)
...
lsarp.host.mpx.heatmap.data.treatment = lsarp.px.all.mat.1
lsarp.host.mpx.heatmap.data.treatment[is.na(lsarp.host.mpx.heatmap.data.treatment)] = 0
lsarp.host.mpx.heatmap.data.treatment =  ((lsarp.host.mpx.heatmap.data.treatment)[rownames(lsarp.host.mpx.heatmap.data.treatment) %in% subset(metadata.lsarp.stool.asv, phase == "treatment")$standard.name,
                                                                                colnames(as.data.frame(lsarp.host.mpx.heatmap.data.treatment)) %in% 
                                                                                  subset(lsarp.lmer.mpx.host.interactions, phase == "treatment" &pval <= 0.1)$taxa]+
                                           (min(lsarp.host.mpx.heatmap.data.treatment[(lsarp.host.mpx.heatmap.data.treatment)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

lsarp.host.mpx.heatmap.data.washout = lsarp.px.all.mat.1
lsarp.host.mpx.heatmap.data.washout[is.na(lsarp.host.mpx.heatmap.data.washout)] = 0
lsarp.host.mpx.heatmap.data.washout = ((lsarp.host.mpx.heatmap.data.washout)[rownames(lsarp.host.mpx.heatmap.data.washout) %in%subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")$standard.name,
                                                                           colnames(as.data.frame(lsarp.host.mpx.heatmap.data.washout)) %in% 
                                                                             subset(lsarp.lmer.mpx.host.interactions,phase == "washout" & pval <= 0.1)$taxa]+
                                        (min(lsarp.host.mpx.heatmap.data.washout[lsarp.host.mpx.heatmap.data.washout!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 




pheatmap::pheatmap((lsarp.host.mpx.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.host.mpx.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.host.mpx.heatmap.data.treatment)/100, max(lsarp.host.mpx.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.host.interactions,phase == "treatment" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.1,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.host.mpx.heatmap.data.treatment))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "Host Protein | Treatment Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_host_mpx_tr_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)


pheatmap::pheatmap((lsarp.host.mpx.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.host.mpx.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.host.mpx.heatmap.data.washout)/100, max(lsarp.host.mpx.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.host.interactions,phase == "washout" & pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("taxa") %>%
                   ...
                   annotation_colors = lsarp.omic.heatmap.mapping.2,
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.host.mpx.heatmap.data.washout))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "Host Protein | Washout Phase",
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_group_host_mpx_wo_heatmap.pdf",
         device = cairo_pdf,
         width=16, height=12)
...
lsarp.omic.heatmap.mapping.resp = list(
  Group = c(`RS` = "#00BFC4", `Placebo` = "#F8766D"),
  RS_Type = rs.colors.type,
  Coefficient = colorRampPalette(c("blue","white", "red"))(100),
  Fiber = colorRampPalette(c("blue","white", "red"))(100),
...
lsarp.asv.heatmap.data.responder.rs = (lsarp.lmer.asv.data[rownames(lsarp.lmer.asv.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "RS")$standard.name,
                                                      colnames(as.data.frame(lsarp.lmer.asv.data)) %in% 
                                                        subset(lsarp.lmer.asv.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.asv.heatmap.data.responder.rs),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.asv.heatmap.data.responder.rs), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.asv.heatmap.data.responder.rs)/100, max(lsarp.asv.heatmap.data.responder.rs), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.asv.resp.interactions, pval <= 0.1 & group == "RS")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     ...
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "RS")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(flare.group = factor(flare.group, levels=c("Remit", "Relapse"))) %>%
                     mutate(Group = group,
                            `Days on product` = lsarp.days,
                            `Response` = `flare.group`,
                            Fiber = ave.fiber) %>%
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.asv.heatmap.data.responder.rs))$new,
                   fontsize_row = 10 - nrow(lsarp.asv.heatmap.data.responder.rs) / 15,
                   fontsize_col = 7,
                   main = "ASV | Responders (RS)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_rs_asv_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)


lsarp.asv.heatmap.data.responder.pl = (lsarp.lmer.asv.data[rownames(lsarp.lmer.asv.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "Placebo")$standard.name,
                                                           colnames(as.data.frame(lsarp.lmer.asv.data)) %in% 
                                                             subset(lsarp.lmer.asv.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.asv.heatmap.data.responder.pl),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.asv.heatmap.data.responder.pl), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.asv.heatmap.data.responder.pl)/100, max(lsarp.asv.heatmap.data.responder.pl), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.asv.resp.interactions, pval <= 0.1 & group == "Placebo")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                     ...
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "Placebo")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(flare.group = factor(flare.group, levels=c("Remit", "Relapse"))) %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.asv.heatmap.data.responder.pl))$new,
                   fontsize_row = 10 - nrow(lsarp.asv.heatmap.data.responder.pl) / 15,
                   fontsize_col = 7,
                   main = "ASV | Responders (Placebo)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_pl_asv_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
lsarp.mgx.heatmap.data.responder.rs = (lsarp.lmer.mgx.data[rownames(lsarp.lmer.mgx.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "RS")$standard.name,
                                                           colnames(as.data.frame(lsarp.lmer.mgx.data)) %in% 
                                                             subset(lsarp.lmer.mgx.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mgx.heatmap.data.responder.rs),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mgx.heatmap.data.responder.rs), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mgx.heatmap.data.responder.rs)/100, max(lsarp.mgx.heatmap.data.responder.rs), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mgx.resp.interactions, pval <= 0.1 & group == "RS")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "RS")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mgx.heatmap.data.responder.rs))$new,
                   fontsize_row = 10 - nrow(lsarp.mgx.heatmap.data.responder.rs) / 15,
                   fontsize_col = 7,
                   main = "Species | Responders (RS)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_rs_mgx_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)


lsarp.mgx.heatmap.data.responder.pl = (lsarp.lmer.mgx.data[rownames(lsarp.lmer.mgx.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "Placebo")$standard.name,
                                                           colnames(as.data.frame(lsarp.lmer.mgx.data)) %in% 
                                                             subset(lsarp.lmer.mgx.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mgx.heatmap.data.responder.pl),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mgx.heatmap.data.responder.pl), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mgx.heatmap.data.responder.pl)/100, max(lsarp.mgx.heatmap.data.responder.pl), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mgx.resp.interactions, pval <= 0.1 & group == "Placebo")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "Placebo")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mgx.heatmap.data.responder.pl))$new,
                   fontsize_row = 10 - nrow(lsarp.mgx.heatmap.data.responder.pl) / 15,
                   fontsize_col = 7,
                   main = "Species | Responders (Placebo)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_pl_mgx_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
lsarp.mpx.kegg.heatmap.data.responder.rs = (lsarp.lmer.mpx.kegg.data[rownames(lsarp.lmer.mpx.kegg.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "RS")$standard.name,
                                                           colnames(as.data.frame(lsarp.lmer.mpx.kegg.data)) %in% 
                                                             subset(lsarp.lmer.mpx.kegg.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.kegg.heatmap.data.responder.rs),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.kegg.heatmap.data.responder.rs), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.kegg.heatmap.data.responder.rs)/100, max(lsarp.mpx.kegg.heatmap.data.responder.rs), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.kegg.resp.interactions, pval <= 0.1 & group == "RS")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "RS")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.kegg.heatmap.data.responder.rs))$new,
                   fontsize_row = 10 - nrow(lsarp.mpx.kegg.heatmap.data.responder.rs) / 15,
                   fontsize_col = 7,
                   main = "Pathway | Responders (RS)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_rs_mpx_kegg_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)


lsarp.mpx.kegg.heatmap.data.responder.pl = (lsarp.lmer.mpx.kegg.data[rownames(lsarp.lmer.mpx.kegg.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "Placebo")$standard.name,
                                                           colnames(as.data.frame(lsarp.lmer.mpx.kegg.data)) %in% 
                                                             subset(lsarp.lmer.mpx.kegg.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.kegg.heatmap.data.responder.pl),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.kegg.heatmap.data.responder.pl), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.kegg.heatmap.data.responder.pl)/100, max(lsarp.mpx.kegg.heatmap.data.responder.pl), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.kegg.resp.interactions, pval <= 0.1 & group == "Placebo")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "Placebo")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.kegg.heatmap.data.responder.pl))$new,
                   fontsize_row = 10 - nrow(lsarp.mpx.kegg.heatmap.data.responder.pl) / 15,
                   fontsize_col = 7,
                   main = "Pathway | Responders (Placebo)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_pl_mpx_kegg_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
lsarp.mpx.cog.heatmap.data.responder.rs = (lsarp.lmer.mpx.cog.data[rownames(lsarp.lmer.mpx.cog.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "RS")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mpx.cog.data)) %in% 
                                                                       subset(lsarp.lmer.mpx.cog.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.cog.heatmap.data.responder.rs),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cog.heatmap.data.responder.rs), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cog.heatmap.data.responder.rs)/100, max(lsarp.mpx.cog.heatmap.data.responder.rs), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cog.resp.interactions, pval <= 0.1 & group == "RS")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "RS")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cog.heatmap.data.responder.rs))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "COG | Responders (RS)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_rs_mpx_cog_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)


lsarp.mpx.cog.heatmap.data.responder.pl = (lsarp.lmer.mpx.cog.data[rownames(lsarp.lmer.mpx.cog.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "Placebo")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mpx.cog.data)) %in% 
                                                                       subset(lsarp.lmer.mpx.cog.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.cog.heatmap.data.responder.pl),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cog.heatmap.data.responder.pl), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cog.heatmap.data.responder.pl)/100, max(lsarp.mpx.cog.heatmap.data.responder.pl), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cog.resp.interactions, pval <= 0.1 & group == "Placebo")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "Placebo")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cog.heatmap.data.responder.pl))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "COG | Responders (Placebo)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_pl_mpx_cog_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
lsarp.mpx.cazy.heatmap.data.responder.rs = (lsarp.lmer.mpx.cazy.data[rownames(lsarp.lmer.mpx.cazy.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "RS")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mpx.cazy.data)) %in% 
                                                                       subset(lsarp.lmer.mpx.cazy.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.cazy.heatmap.data.responder.rs),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cazy.heatmap.data.responder.rs), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cazy.heatmap.data.responder.rs)/100, max(lsarp.mpx.cazy.heatmap.data.responder.rs), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cazy.resp.interactions, pval <= 0.1 & group == "RS")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "RS")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cazy.heatmap.data.responder.rs))$new,
                   fontsize_row = 10 - nrow(lsarp.mpx.cazy.heatmap.data.responder.rs) / 15,
                   fontsize_col = 7,
                   main = "CAZyme | Responders (RS)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_rs_mpx_cazy_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)


lsarp.mpx.cazy.heatmap.data.responder.pl = (lsarp.lmer.mpx.cazy.data[rownames(lsarp.lmer.mpx.cazy.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "Placebo")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mpx.cazy.data)) %in% 
                                                                       subset(lsarp.lmer.mpx.cazy.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.cazy.heatmap.data.responder.pl),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.cazy.heatmap.data.responder.pl), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.cazy.heatmap.data.responder.pl)/100, max(lsarp.mpx.cazy.heatmap.data.responder.pl), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mpx.cazy.resp.interactions, pval <= 0.1 & group == "Placebo")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "Placebo")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.cazy.heatmap.data.responder.pl))$new,
                   fontsize_row = 10 - nrow(lsarp.mpx.cazy.heatmap.data.responder.pl) / 15,
                   fontsize_col = 7,
                   main = "CAZyme | Responders (Placebo)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_pl_mpx_cazy_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
lsarp.mbx.heatmap.data.responder.rs = (lsarp.lmer.mbx.data[rownames(lsarp.lmer.mbx.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "RS")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mbx.data)) %in% 
                                                                       subset(lsarp.lmer.mbx.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mbx.heatmap.data.responder.rs),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mbx.heatmap.data.responder.rs), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mbx.heatmap.data.responder.rs)/100, max(lsarp.mbx.heatmap.data.responder.rs), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mbx.resp.interactions, pval <= 0.1 & group == "RS")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "RS")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mbx.heatmap.data.responder.rs))$new,
                   fontsize_row = 10 - nrow(lsarp.mbx.heatmap.data.responder.rs) / 15,
                   fontsize_col = 7,
                   main = "Metabolite | Responders (RS)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_rs_mbx_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)


lsarp.mbx.heatmap.data.responder.pl = (lsarp.lmer.mbx.data[rownames(lsarp.lmer.mbx.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "Placebo")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mbx.data)) %in% 
                                                                       subset(lsarp.lmer.mbx.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mbx.heatmap.data.responder.pl),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mbx.heatmap.data.responder.pl), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mbx.heatmap.data.responder.pl)/100, max(lsarp.mbx.heatmap.data.responder.pl), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.mbx.resp.interactions, pval <= 0.1 & group == "Placebo")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "Placebo")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mbx.heatmap.data.responder.pl))$new,
                   fontsize_row = 10 - nrow(lsarp.mbx.heatmap.data.responder.pl) / 15,
                   fontsize_col = 7,
                   main = "Metabolite | Responders (Placebo)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_pl_mbx_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
lsarp.mpx.host.heatmap.data.responder.rs = (lsarp.lmer.mpx.host.data[rownames(lsarp.lmer.mpx.host.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "RS")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mpx.host.data)) %in% 
                                                                       subset(lsarp.lmer.host.mpx.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.host.heatmap.data.responder.rs),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.host.heatmap.data.responder.rs), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.host.heatmap.data.responder.rs)/100, max(lsarp.mpx.host.heatmap.data.responder.rs), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.host.mpx.resp.interactions, pval <= 0.1 & group == "RS")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "RS")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     tibble::column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(flare.group = factor(flare.group, levels=c("Remit", "Relapse"))) %>%
                     mutate(Group = group,
                            `Days on product` = lsarp.days,
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.host.heatmap.data.responder.rs))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "Host Protein | Responders (RS)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_rs_mpx_host_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)


lsarp.mpx.host.heatmap.data.responder.pl = (lsarp.lmer.mpx.host.data[rownames(lsarp.lmer.mpx.host.data) %in% subset(metadata.lsarp.stool.asv, phase == "treatment" & group == "Placebo")$standard.name,
                                                                     colnames(as.data.frame(lsarp.lmer.mpx.host.data)) %in% 
                                                                       subset(lsarp.lmer.host.mpx.resp.interactions, pval <= 0.1)$taxa]) %>%
  log10() %>% as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((lsarp.mpx.host.heatmap.data.responder.pl),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(lsarp.mpx.host.heatmap.data.responder.pl), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(lsarp.mpx.host.heatmap.data.responder.pl)/100, max(lsarp.mpx.host.heatmap.data.responder.pl), length.out=floor(100/2))),
                   annotation_row = subset(lsarp.lmer.host.mpx.resp.interactions, pval <= 0.1 & group == "Placebo")%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     tibble::remove_rownames() %>% 
                   ...
                   annotation_col = subset(lsarp.metadata.responders, phase == "treatment" & group == "Placebo")[,c("HM", "lsarp.days", "group", "RS_Name", "ave.fiber", "standard.name", "flare.group")] %>%
                     ...
                     dplyr::select(Group, Response, `Days on product`, Fiber, RS_Type),
                   labels_col = sapply(colnames(lsarp.asv.heatmap.data.treatment), deid.fun),
                   labels_row = name.shortener(rownames(lsarp.mpx.host.heatmap.data.responder.pl))$new,
                   fontsize_row = 2,
                   fontsize_col = 7,
                   main = "Host Protein | Responders (Placebo)",
                   annotation_colors = lsarp.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./lsarp_plots_deidentified/lsarp_response_pl_mpx_host_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
openxlsx::write.xlsx(
  list(
    `st 2 asv_tr` = lsarp.lmer.asv.interactions %>% subset(phase == "treatment") %>% mutate(data.type = "ASV", phase="treatment"),
    `st 3 mgx_tr` = lsarp.lmer.mgx.interactions %>% subset(phase == "treatment") %>% mutate(data.type = "Species", phase="treatment"),
    `st 4 kegg_tr` = lsarp.lmer.mpx.kegg.interactions %>% subset(phase == "treatment") %>% mutate(data.type = "Pathway", phase="treatment"),
    `st 5 cog_tr` = lsarp.lmer.mpx.cog.interactions %>% subset(phase == "treatment") %>% mutate(data.type = "COG", phase="treatment"),
    `st 6 cazy_tr` = lsarp.lmer.mpx.cazy.interactions %>% subset(phase == "treatment") %>% mutate(data.type = "CAZyme", phase="treatment"),
    `st 7 mbx_tr` = lsarp.lmer.mbx.interactions %>% subset(phase == "treatment") %>% mutate(data.type = "Metabolite", phase="treatment"),
    
    `st 8 asv_wo` = lsarp.lmer.asv.interactions %>% subset(phase == "washout") %>% mutate(data.type = "ASV", phase="washout"),
    `st 9 mgx_wo` = lsarp.lmer.mgx.interactions %>% subset(phase == "washout") %>% mutate(data.type = "Species", phase="washout"),
    `st 10 kegg_wo` = lsarp.lmer.mpx.kegg.interactions %>% subset(phase == "washout") %>% mutate(data.type = "Pathway", phase="washout"),
    `st 11 cog_wo` = lsarp.lmer.mpx.cog.interactions %>% subset(phase == "washout") %>% mutate(data.type = "COG", phase="washout"),
    `st 12 cazy_wo` = lsarp.lmer.mpx.cazy.interactions %>% subset(phase == "washout") %>% mutate(data.type = "CAZyme", phase="washout"),
    `st 13 mbx_wo` = lsarp.lmer.mbx.interactions %>% subset(phase == "washout") %>% mutate(data.type = "Metabolite", phase="washout"),
    
    `st 14 host_mpx_tr` = lsarp.lmer.mpx.host.interactions %>% subset(phase == "treatment") %>% mutate(data.type = "Host Protein", phase="treatment"),
    `st 15 host_mpx_wo` = lsarp.lmer.mpx.host.interactions %>% subset(phase == "washout") %>% mutate(data.type = "Host Protein", phase="washout"),

    `st 16 asv_resp_rs` = lsarp.lmer.asv.resp.interactions %>% subset(group == "RS") %>% mutate(data.type = "ASV"),
    `st 17 asv_resp_pl` = lsarp.lmer.asv.resp.interactions %>% subset(group == "Placebo") %>% mutate(data.type = "ASV"),
    `st 18 mgx_resp_rs` = lsarp.lmer.mgx.resp.interactions %>% subset(group == "RS") %>% mutate(data.type = "Species"),
    `st 19 mgx_resp_pl` = lsarp.lmer.mgx.resp.interactions %>% subset(group == "Placebo") %>% mutate(data.type = "Species"),
    `st 20 kegg_resp_rs` = lsarp.lmer.mpx.kegg.resp.interactions %>% subset(group == "RS") %>% mutate(data.type = "Pathway"),
    `st 21 kegg_resp_pl` = lsarp.lmer.mpx.kegg.resp.interactions %>% subset(group == "Placebo") %>% mutate(data.type = "Pathway"),
    `st 22 cog_resp_rs` = lsarp.lmer.mpx.cog.resp.interactions %>% subset(group == "RS") %>% mutate(data.type = "COG"),
    `st 23 cog_resp_pl` = lsarp.lmer.mpx.cog.resp.interactions %>% subset(group == "Placebo") %>% mutate(data.type = "COG"),
    `st 24 cazy_resp_rs` = lsarp.lmer.mpx.cazy.resp.interactions %>% subset(group == "RS") %>% mutate(data.type = "CAZyme"),
    `st 25 cazy_resp_pl` = lsarp.lmer.mpx.cazy.resp.interactions %>% subset(group == "Placebo") %>% mutate(data.type = "CAZyme"),
    `st 26 mbx_resp_rs` = lsarp.lmer.mbx.resp.interactions %>% subset(group == "RS") %>% mutate(data.type = "Metabolite"),
    `st 27 mbx_resp_pl` = lsarp.lmer.mbx.resp.interactions %>% subset(group == "Placebo") %>% mutate(data.type = "Metabolite"),
    `st 28 host_resp_rs` = lsarp.lmer.host.mpx.resp.interactions %>% subset(group == "RS") %>% mutate(data.type = "Host Protein"),
    `st 29 host_resp_pl` = lsarp.lmer.host.mpx.resp.interactions %>% subset(group == "Placebo") %>% mutate(data.type = "Host Protein")
    
    #`st X other` = ``
  ),
  
  "lsarp_paper/lsarp_heatmap_source.xlsx")

openxlsx::write.xlsx(
  list(`st 35 baseline` = lsarp.rf.models.importances.wilcox),
  "lsarp_paper/lsarp_supp_wilcox.xlsx")
...
(metadata.lsarp.stool.plot +
  ...
  ggsave(filename="./lsarp_plots_deidentified/2026_05_15_lsarp_1_stools_survival_DI.pdf",
         width=14, height=5,device = cairo_pdf)

metadata.lsarp.stool.plot %>%
  ggsave(filename="./lsarp_plots_deidentified/2026_07_28_lsarp_1_stools_DI.pdf",
...
(lsarp.cd.rs.selections.placebo.plot+
  ...
  ggsave(filename="./lsarp_plots_deidentified/2026_05_16_lsarp_1_rs_selections_DI.pdf",
         width=16, height=8, device = cairo_pdf)


(metadata.lsarp.but.i.plot+metadata.lsarp.fcal.plot)%>%
  ggsave(filename="./lsarp_plots/2026_07_28_lsarp_primary_plots.pdf",
...
(metadata.lsarp.stool.asv.richness.plot+
    ...
    metadata.lsarp.stool.asv.fd.plot+
      ...
      metadata.lsarp.mucin.plot+
      metadata.lsarp.starch.mucin.plot+
  patchwork::plot_layout(nrow=5)) %>%
  ggsave(filename="./lsarp_plots/2026_07_28_lsarp_1_secondary.pdf",
...
# old
(patchwork::free(patchwork::plot_spacer()+
    lsarp.metadata.responders.ttest.but.plot+
    lsarp.metadata.responders.ttest.fcal.plot+
    metadata.lsarp.but.i.resp.plot+
       metadata.lsarp.fcal.resp.plot+
       ...
       metadata.lsarp.mucin.resp.plot+
       metadata.lsarp.starch.mucin.resp.plot+
    patchwork::plot_layout(nrow=5), type="label")) %>%
  ggsave(filename="./lsarp_plots/2026_03_01_lsarp_2_response_standard.pdf",
         width=14, height=13, device = cairo_pdf)
# new
cowplot::plot_grid((metadata.lsarp.but.i.resp.plot+
                   metadata.lsarp.fcal.resp.plot))%>%
                    ggsave(filename="./lsarp_plots/2026_07_28_lsarp_2_primary.pdf",
...
(metadata.lsarp.stool.asv.richness.resp.plot+
    ...
    metadata.lsarp.starch.mucin.resp.plot+
    patchwork::plot_layout(nrow=5)) %>%
  ggsave(filename="./lsarp_plots/2026_07_28_lsarp_2_secondary.pdf",
...
(((patchwork::free(rf.pca.p1, type="label")/
  ...
  (lsarp.rf.selected.importances.output.plot))/
  (lsarp.ml.predictors.over.time.plot))+
  patchwork::plot_layout(heights=c(0.7,3,2)))) %>%
  ggsave(filename="./lsarp_plots/2026_08_18_lsarp_2_response_predictors.pdf",
...
(lsarp.asv.pcoa.plot+
  ...
  ggsave(filename="./lsarp_plots/2026_03_01_lsarp_supp_group_pca.pdf",
...
(lsarp.lmer.asv.resp.interactions.plot+
    ...
    #lsarp.lmer.host.mpx.resp.interactions.plot+
    patchwork::plot_layout(nrow=3))%>%
  ggsave(filename="./lsarp_plots/2026_07_28_lsarp_supp_responders_interaction_volcano.pdf",
         width=12, height=10, device = cairo_pdf)
...
(lsarp.rf.models.importances.wilcox.plot+
  lsarp.rf.models.importances.ttest.plot) %>%
  ggsave(filename="./lsarp_plots/2026_03_06_lsarp_supp_rf_wilcoxon.pdf",
...
# fcal
lsarp.stats.fcal.treatment.simple = lmerTest::lmer(scale(log10(fcal)) ~ group*scale(lsarp.days) + (1|HM),
                                            subset(metadata.lsarp.stool, phase %in% c("treatment"))%>%
                                              subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.fcal.washout.simple = lmerTest::lmer(scale(log10(fcal)) ~ group*scale(lsarp.days) + (1|HM),
                                          subset(metadata.lsarp.stool, phase == "washout" | rs.end == "rs.end") %>%
                                            subset(flare != "flare") %>%  subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Stool moisture
lsarp.stats.water.treatment.simple = lmerTest::lmer(scale((stool_water_perc)) ~ group*scale(lsarp.days) + (1|HM),
                                             subset(metadata.lsarp.stool, phase %in% c("treatment"))%>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.water.washout.simple = lmerTest::lmer(scale((stool_water_perc)) ~ group*scale(lsarp.days) + (1|HM),
                                           subset(metadata.lsarp.stool, phase %in% "washout" | rs.end == "rs.end")%>%
                                             subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Bacterial load
lsarp.stats.load.treatment.simple = lmerTest::lmer(scale(load.asv) ~ group*scale(lsarp.days) + (1|HM),
                                            subset(metadata.lsarp.stool.asv, phase %in% "treatment") %>%
                                              subset(flare != "flare" & !HM %in% excluded.by.dave)) %>%  summary() %>% coef()

lsarp.stats.load.washout.simple = lmerTest::lmer(scale(load.asv) ~ group*scale(lsarp.days) + (1|HM),
                                          subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end") %>% subset(flare != "flare") %>%
                                            subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Richness
lsarp.stats.richness.treatment.simple = lmerTest::lmer(scale(log10(richness)) ~ group*scale(lsarp.days) + (1|HM),
                                                subset(metadata.lsarp.stool.asv, phase == "treatment") %>%
                                                  subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.richness.washout.simple = lmerTest::lmer(scale(log10(richness)) ~ group*scale(lsarp.days) + (1|HM),
                                              subset(metadata.lsarp.stool.asv,  phase == "washout" | rs.end == "rs.end") %>%
                                                subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Shannon
lsarp.stats.shannon.treatment.simple = lmerTest::lmer(scale((shannon)) ~ group*scale(lsarp.days) + (1|HM),
                                               subset(metadata.lsarp.stool.asv, phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.shannon.washout.simple = lmerTest::lmer(scale((shannon)) ~ group*scale(lsarp.days) + (1|HM),
                                             subset(metadata.lsarp.stool.asv,  phase == "washout" | rs.end == "rs.end") %>%
                                               subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Functional redundancy
lsarp.stats.fr.treatment.simple = lmerTest::lmer(scale((fr)) ~ group*scale(lsarp.days) +(1|HM),
                                          subset(metadata.lsarp.stool.asv, phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.fr.washout.simple = lmerTest::lmer(scale((fr)) ~ group*scale(lsarp.days) +  (1|HM),
                                        subset(metadata.lsarp.stool.asv,  phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare") %>%
                                          subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Functional richness
lsarp.stats.fd.treatment.simple = lmerTest::lmer(scale((fd)) ~ group*scale(lsarp.days) +(1|HM),
                                                 subset(metadata.lsarp.stool.asv, phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.fd.washout.simple = lmerTest::lmer(scale((fd)) ~ group*scale(lsarp.days) +  (1|HM),
                                               subset(metadata.lsarp.stool.asv,  phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare") %>%
                                                 subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Butyrogens
lsarp.stats.but.i.treatment.simple = lmerTest::lmer(scale(log10(but.i)) ~ group*scale(lsarp.days) + (1|HM),
                                             subset(metadata.lsarp.stool.asv, phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.but.i.washout.simple = lmerTest::lmer(scale(log10(but.i)) ~ group*scale(lsarp.days) + (1|HM),
                                           subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")  %>%
                                             subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Beta
stats.beta.between.treatment.simple = lmerTest::lmer(scale((between.beta)) ~ group*scale(lsarp.days) + (1|HM),
                                              subset(beta.trajectory.data, phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

stats.beta.between.washout.simple = lmerTest::lmer(scale((between.beta)) ~ group*scale(lsarp.days) + (1|HM),
                                            subset(beta.trajectory.data,  phase == "washout" | rs.end == "rs.end") %>%
                                              subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Starch
lsarp.stats.starch.treatment.simple = lmerTest::lmer(scale(log10(starch)) ~ group*scale(lsarp.days) + (1|HM) + (1|plate),
                                              subset(metadata.lsarp.stool.asv, phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.starch.washout.simple = lmerTest::lmer(scale(log10(starch)) ~ group*scale(lsarp.days) + (1|HM) + (1|plate),
                                            subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave))  %>% summary() %>% coef()

# Mucin
lsarp.stats.mucin.treatment.simple = lmerTest::lmer(scale(log10(mucin)) ~ group*scale(lsarp.days) + (1|HM) + (1|plate),
                                                     subset(metadata.lsarp.stool.asv, phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.mucin.washout.simple = lmerTest::lmer(scale(log10(mucin)) ~ group*scale(lsarp.days) + (1|HM) + (1|plate),
                                                   subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave))  %>% summary() %>% coef()


# Starch:Mucin
lsarp.stats.starch.mucin.treatment.simple = lmerTest::lmer(scale((starch.mucin)) ~ group*scale(lsarp.days) + (1|HM) + (1|plate),
                                                     subset(metadata.lsarp.stool.asv, phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.starch.mucin.washout.simple = lmerTest::lmer(scale((starch.mucin)) ~ group*scale(lsarp.days) + (1|HM) + (1|plate),
                                                   subset(metadata.lsarp.stool.asv, phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave))  %>% summary() %>% coef()
...
# fcal
lsarp.resp.stats.fcal.treatment.simple = lmerTest::lmer(scale(log10(fcal)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                 subset(lsarp.metadata.responders, group %in% c("RS")) %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.fcal.placebo.simple = lmerTest::lmer(scale(log10(fcal)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                               subset(lsarp.metadata.responders, group %in% c("Placebo")) %>%
                                                 subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Stool moisture
lsarp.resp.stats.water.treatment.simple = lmerTest::lmer(scale(stool_water_perc) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                  subset(lsarp.metadata.responders, group %in% c("RS")) %>%
                                                    subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.water.placebo.simple = lmerTest::lmer(scale(stool_water_perc) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                subset(lsarp.metadata.responders, group %in% c("Placebo")) %>%
                                                  subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Bacterial load
lsarp.resp.stats.load.treatment.simple = lmerTest::lmer(scale(load.asv) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                 subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                   subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.load.placebo.simple = lmerTest::lmer(scale(load.asv) ~ scale(lsarp.days)*flare.group + (1|HM),
                                               subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                 subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Richness
lsarp.resp.stats.richness.resp.treatment = lmerTest::lmer(scale(log10(richness)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                          subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                            subset(flare!="flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.richness.resp.placebo = lmerTest::lmer(scale(log10(richness)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                        subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                          subset(flare!="flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

# Shannon
lsarp.resp.stats.shannon.resp.treatment.simple = lmerTest::lmer(scale((shannon)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                          subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                            subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.shannon.resp.placebo.simple = lmerTest::lmer(scale((shannon)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                        subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                          subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Functional redundancy
lsarp.resp.stats.fd.treatment.simple = lmerTest::lmer(scale(fd) ~ scale(lsarp.days)*flare.group + (1|HM),
                                               subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                 subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.fd.placebo.simple = lmerTest::lmer(scale(fd) ~ scale(lsarp.days)*flare.group + (1|HM),
                                             subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                               subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

# Functional richness
lsarp.resp.stats.fr.treatment.simple = lmerTest::lmer(scale(fr) ~ scale(lsarp.days)*flare.group + (1|HM),
                                               subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                 subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.fr.placebo.simple = lmerTest::lmer(scale(fr) ~ scale(lsarp.days)*flare.group + (1|HM),
                                             subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                               subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

# Butyrogens
lsarp.resp.stats.but.i.treatment.simple = lmerTest::lmer(scale(log10(but.i)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                  subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                    subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.but.i.placebo.simple = lmerTest::lmer(scale(log10(but.i)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                  subset(flare != "flare"& !HM %in% excluded.by.dave))%>% summary() %>% coef()

# Beta
lsarp.resp.stats.between.beta.treatment.simple = lmerTest::lmer(scale(log10(between.beta)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                         subset(beta.trajectory.resp.data, group %in% c("RS")) %>%
                                                           subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.between.beta.placebo.simple = lmerTest::lmer(scale(log10(between.beta)) ~ scale(lsarp.days)*flare.group + (1|HM),
                                                       subset(beta.trajectory.resp.data, group %in% c("Placebo")) %>%
                                                         subset(flare != "flare"& !HM %in% excluded.by.dave))%>% summary() %>% coef()

# Starch
lsarp.resp.stats.starch.treatment.simple = lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days)*flare.group + (1|HM) + (1|plate),
                                                   subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                     subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.starch.placebo.simple = lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days)*flare.group + (1|HM) + (1|plate),
                                                 subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                   subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Mucin
lsarp.resp.stats.mucin.treatment.simple = lmerTest::lmer(scale(log10(mucin)) ~ scale(lsarp.days)*flare.group + (1|HM) + (1|plate),
                                                          subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                            subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.mucin.placebo.simple = lmerTest::lmer(scale(log10(mucin)) ~ scale(lsarp.days)*flare.group + (1|HM) + (1|plate),
                                                        subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                          subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Starch:Mucin
lsarp.resp.stats.starch.mucin.treatment.simple = lmerTest::lmer(scale((starch.mucin)) ~ scale(lsarp.days)*flare.group + (1|HM) + (1|plate),
                                                          subset(lsarp.metadata.responders.asv, group %in% c("RS")) %>%
                                                            subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.starch.mucin.placebo.simple = lmerTest::lmer(scale((starch.mucin)) ~ scale(lsarp.days)*flare.group + (1|HM) + (1|plate),
                                                        subset(lsarp.metadata.responders.asv, group %in% c("Placebo")) %>%
                                                          subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()
...
# PP
redcap.lsarp.coxph.pp.wcpdai = survival::coxph(Surv(time.to.event, status.pre) ~ group + wpcdai, 
                                        data = subset(redcap.lsarp.wpcdai.pp, HM != "HM0962") %>%
                                          merge(lsarp.new.table.for.model[,c("HM", "wpcdai")]))
redcap.lsarp.coxph.pp.wcpdai <- broom::tidy(redcap.lsarp.coxph.pp.wcpdai, exponentiate = TRUE, conf.int = TRUE) %>%
  mutate(HR = round(estimate, 3),
         `95% CI` = paste0(round(conf.low, 3), " - ", round(conf.high, 3)),
         `p-value` = signif(p.value, 3)) %>%
  dplyr::select(term, HR, `95% CI`, `p-value`) %>% as.data.frame()
# more sig after controlling for wPCDAI

lsarp.new.table.for.model = lsarp.new.table %>%
  mutate(HM = substr(HM, 1, 6),
         wpcdai = wpcdai_at_rs)

# fcal
lsarp.stats.fcal.treatment.wpcdai = lmerTest::lmer(scale(log10(fcal)) ~ group*scale(lsarp.days) + wpcdai + (1|HM),
                                                   subset(merge(metadata.lsarp.stool,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase %in% c("treatment"))%>%
                                                     subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.fcal.washout.wpcdai = lmerTest::lmer(scale(log10(fcal)) ~ group*scale(lsarp.days) + wpcdai + (1|HM),
                                                 subset(merge(metadata.lsarp.stool,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "washout" | rs.end == "rs.end") %>%
                                                   subset(flare != "flare") %>%  subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Stool moisture
lsarp.stats.water.treatment.wpcdai = lmerTest::lmer(scale((stool_water_perc)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                    subset(merge(metadata.lsarp.stool,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase %in% c("treatment"))%>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.water.washout.wpcdai = lmerTest::lmer(scale((stool_water_perc)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                  subset(merge(metadata.lsarp.stool,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase %in% "washout" | rs.end == "rs.end")%>%
                                                    subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Bacterial load
lsarp.stats.load.treatment.wpcdai = lmerTest::lmer(scale(load.asv) ~ group*scale(lsarp.days) +wpcdai+ (1|HM),
                                                   subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase %in% "treatment") %>%
                                                     subset(flare != "flare" & !HM %in% excluded.by.dave)) %>%  summary() %>% coef()

lsarp.stats.load.washout.wpcdai = lmerTest::lmer(scale(load.asv) ~ group*scale(lsarp.days) +wpcdai+ (1|HM),
                                                 subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "washout" | rs.end == "rs.end") %>% subset(flare != "flare") %>%
                                                   subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Richness
lsarp.stats.richness.treatment.wpcdai = lmerTest::lmer(scale(log10(richness)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                       subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment") %>%
                                                         subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.richness.washout.wpcdai = lmerTest::lmer(scale(log10(richness)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                     subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"),  phase == "washout" | rs.end == "rs.end") %>%
                                                       subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Shannon
lsarp.stats.shannon.treatment.wpcdai = lmerTest::lmer(scale((shannon)) ~ group*scale(lsarp.days) +wpcdai+ (1|HM),
                                                      subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.shannon.washout.wpcdai = lmerTest::lmer(scale((shannon)) ~ group*scale(lsarp.days) +wpcdai+ (1|HM),
                                                    subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"),  phase == "washout" | rs.end == "rs.end") %>%
                                                      subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Functional redundancy
lsarp.stats.fr.treatment.wpcdai = lmerTest::lmer(scale((fr)) ~ group*scale(lsarp.days) +wpcdai+(1|HM),
                                                 subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.fr.washout.wpcdai = lmerTest::lmer(scale((fr)) ~ group*scale(lsarp.days) +  wpcdai+(1|HM),
                                               subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"),  phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare") %>%
                                                 subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Functional richness
lsarp.stats.fd.treatment.wpcdai = lmerTest::lmer(scale((fd)) ~ group*scale(lsarp.days) +wpcdai+(1|HM),
                                                 subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.fd.washout.wpcdai = lmerTest::lmer(scale((fd)) ~ group*scale(lsarp.days) +  wpcdai+(1|HM),
                                               subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"),  phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare") %>%
                                                 subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Butyrogens
lsarp.stats.but.i.treatment.wpcdai = lmerTest::lmer(scale(log10(but.i)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                    subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.but.i.washout.wpcdai = lmerTest::lmer(scale(log10(but.i)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                  subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "washout" | rs.end == "rs.end")  %>%
                                                    subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Beta
stats.beta.between.treatment.wpcdai = lmerTest::lmer(scale((between.beta)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                     subset(merge(beta.trajectory.data,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

stats.beta.between.washout.wpcdai = lmerTest::lmer(scale((between.beta)) ~ group*scale(lsarp.days) + wpcdai+(1|HM),
                                                   subset(merge(beta.trajectory.data,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"),  phase == "washout" | rs.end == "rs.end") %>%
                                                     subset(flare != "flare") %>% subset(!HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Starch
lsarp.stats.starch.treatment.wpcdai = lmerTest::lmer(scale(log10(starch)) ~ group*scale(lsarp.days) + wpcdai+(1|HM) + (1|plate),
                                                     subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.starch.washout.wpcdai = lmerTest::lmer(scale(log10(starch)) ~ group*scale(lsarp.days) + wpcdai+(1|HM) + (1|plate),
                                                   subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave))  %>% summary() %>% coef()

# Mucin
lsarp.stats.mucin.treatment.wpcdai = lmerTest::lmer(scale(log10(mucin)) ~ group*scale(lsarp.days) + wpcdai+(1|HM) + (1|plate),
                                                    subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.mucin.washout.wpcdai = lmerTest::lmer(scale(log10(mucin)) ~ group*scale(lsarp.days) + wpcdai+(1|HM) + (1|plate),
                                                  subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave))  %>% summary() %>% coef()


# Starch:Mucin
lsarp.stats.starch.mucin.treatment.wpcdai = lmerTest::lmer(scale((starch.mucin)) ~ group*scale(lsarp.days) + wpcdai+(1|HM) + (1|plate),
                                                           subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "treatment")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.stats.starch.mucin.washout.wpcdai = lmerTest::lmer(scale((starch.mucin)) ~ group*scale(lsarp.days) + wpcdai+(1|HM) + (1|plate),
                                                         subset(merge(metadata.lsarp.stool.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), phase == "washout" | rs.end == "rs.end")  %>% subset(flare != "flare"& !HM %in% excluded.by.dave))  %>% summary() %>% coef()


# :: wPCDAI models - Response ---------------------------------------------

# fcal (still sig)
lsarp.resp.stats.fcal.treatment.wpcdai = lmerTest::lmer(scale(log10(fcal)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                        subset(merge(lsarp.metadata.responders,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>% subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.fcal.placebo.wpcdai = lmerTest::lmer(scale(log10(fcal)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                      subset(merge(lsarp.metadata.responders,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                        subset(flare != "flare" & !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Stool moisture
lsarp.resp.stats.water.treatment.wpcdai = lmerTest::lmer(scale(stool_water_perc) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                         subset(merge(lsarp.metadata.responders,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                           subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.water.placebo.wpcdai = lmerTest::lmer(scale(stool_water_perc) ~ scale(lsarp.days)*flare.group +wpcdai+ (1|HM),
                                                       subset(merge(lsarp.metadata.responders,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                         subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Bacterial load
lsarp.resp.stats.load.treatment.wpcdai = lmerTest::lmer(scale(load.asv) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                        subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                          subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.load.placebo.wpcdai = lmerTest::lmer(scale(load.asv) ~ scale(lsarp.days)*flare.group +wpcdai+ (1|HM),
                                                      subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                        subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Richness
lsarp.resp.stats.richness.resp.treatment = lmerTest::lmer(scale(log10(richness)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                          subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                            subset(flare!="flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.richness.resp.placebo = lmerTest::lmer(scale(log10(richness)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                        subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                          subset(flare!="flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

# Shannon
lsarp.resp.stats.shannon.resp.treatment.wpcdai = lmerTest::lmer(scale((shannon)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                                subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                                  subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.shannon.resp.placebo.wpcdai = lmerTest::lmer(scale((shannon)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                              subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                                subset(flare!="flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Functional redundancy
lsarp.resp.stats.fd.treatment.wpcdai = lmerTest::lmer(scale(fd) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                      subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                        subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.fd.placebo.wpcdai = lmerTest::lmer(scale(fd) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                    subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                      subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

# Functional richness (sig)
lsarp.resp.stats.fr.treatment.wpcdai = lmerTest::lmer(scale(fr) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                      subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                        subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

lsarp.resp.stats.fr.placebo.wpcdai = lmerTest::lmer(scale(fr) ~ scale(lsarp.days)*flare.group +wpcdai+ (1|HM),
                                                    subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                      subset(flare != "flare"& !HM %in% excluded.by.dave)) %>%
  summary() %>% coef()

# Butyrogens (still sig)
lsarp.resp.stats.but.i.treatment.wpcdai = lmerTest::lmer(scale(log10(but.i)) ~ scale(lsarp.days)*flare.group +wpcdai+ (1|HM),
                                                         subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                           subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.but.i.placebo.wpcdai = lmerTest::lmer(scale(log10(but.i)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                       subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                         subset(flare != "flare"& !HM %in% excluded.by.dave))%>% summary() %>% coef()

# Beta
lsarp.resp.stats.between.beta.treatment.wpcdai = lmerTest::lmer(scale(log10(between.beta)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                                subset(merge(beta.trajectory.resp.data,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                                  subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.between.beta.placebo.wpcdai = lmerTest::lmer(scale(log10(between.beta)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM),
                                                              subset(merge(beta.trajectory.resp.data,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                                subset(flare != "flare"& !HM %in% excluded.by.dave))%>% summary() %>% coef()

# Starch
lsarp.resp.stats.starch.treatment.wpcdai = lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM) + (1|plate),
                                                          subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                            subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.starch.placebo.wpcdai = lmerTest::lmer(scale(log10(starch)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM) + (1|plate),
                                                        subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                          subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Mucin
lsarp.resp.stats.mucin.treatment.wpcdai = lmerTest::lmer(scale(log10(mucin)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM) + (1|plate),
                                                         subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                           subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.mucin.placebo.wpcdai = lmerTest::lmer(scale(log10(mucin)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM) + (1|plate),
                                                       subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                         subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

# Starch:Mucin
lsarp.resp.stats.starch.mucin.treatment.wpcdai = lmerTest::lmer(scale((starch.mucin)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM) + (1|plate),
                                                                subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("RS")) %>%
                                                                  subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()

lsarp.resp.stats.starch.mucin.placebo.wpcdai = lmerTest::lmer(scale((starch.mucin)) ~ scale(lsarp.days)*flare.group + wpcdai+(1|HM) + (1|plate),
                                                              subset(merge(lsarp.metadata.responders.asv,lsarp.new.table.for.model[,c("HM", "wpcdai")], by="HM"), group %in% c("Placebo")) %>%
                                                                subset(flare != "flare"& !HM %in% excluded.by.dave)) %>% summary() %>% coef()
...
nrow(lsarp.mgx.resp.pcoa.df)


# MPX numbers 12 v 14
lsarp.cazy.lsarp.pca.df[,c("HM", "Group")] %>% table()
lsarp.cazy.lsarp.pca.df[,c("HM", "Group")] %>% distinct() %>% dplyr::select(Group) %>% table()
nrow(lsarp.cazy.lsarp.pca.df)
                           ...
                           mutate(datatype = ifelse(variable %in% lsarp.mgx.lfc$variable, "Species",
                                                    ifelse(variable %in% lsarp.mpx.kegg.lfc$variable, "Pathway", 
                                                           ifelse(variable %in% lsarp.mpx.cog.lfc$variable, "COG", 
                                                                  ifelse(variable %in% lsarp.mpx.cazy.lfc$variable, "CAZyme", "other"))))))) %>% as.data.frame()
                            ...
                            ifelse(feature %in% colnames(lsarp.mgx.taxa), "Species", 
                                   ifelse(feature %in% colnames(lsarp.cd.mpx.kegg.mat), "Pathway", 
                                          ifelse(feature %in% colnames(lsarp.cd.mpx.cog.mat), "COG", 
                                                 ifelse(feature %in% colnames(lsarp.cd.mpx.cazy.mat), "CAZyme", "Fecal calprotectin"))))))
...
ggplot(lsarp.rsf.validation.output%>%
         ...
         mutate(data.set = factor(data.set, levels=c("ASV", "MGX", "Pathway", "COG", "CAZyme", "ALL"))),
