...
v.date = "2026_03_03"
# save.image("./2026_03_03_lsarp_uc_analysis.Renv")

# load("./2026_03_03_lsarp_uc_analysis.Renv")

# :: load packages --------------------------------------------------------
library("ggplot2"); library("dplyr"); library("tidyverse")

# load de-identification codes
lsarp.uc.deid = read.csv("./lsarp_plots_deidentified/2026_05_12_lsarp_uc_codes.csv")
...
pval.positioner = function(
  ...
  if(type == "group"){
    ...
    treatment.range = c(0, max(subset(data, phase == "onRS")$lsarp.days))
    placebo.range = c(max(subset(data, phase == "onRS")$lsarp.days), max(subset(data,  phase == "postRS")$lsarp.days))
  ...
  if(type == "response"){
    # 1. calculate x ranges for each phase:
    range = range(subset(data, phase %in% c("preRS", "onRS"))$lsarp.days)
    # 2. calculate center value (mean)
...
deid.fun = function(x) {
  ...
  new_parts <- sapply(parts, function(p) {
    ...
    replace_map = setNames(lsarp.uc.deid$code, lsarp.uc.deid$HM)
...
lsarp.uc.stool.plot = ggplot(lsarp.uc.redcap %>% subset(!HM %in% excluded.lsarp.uc) %>%
                               subset(standard.name != "HM0940-STL-03") %>%
                               # remove post-flare
  ...
  # deid
  scale_y_discrete(labels = lsarp.uc.deid$code, 
                   breaks = lsarp.uc.deid$HM) +
...
lsarp.uc.fiber.comparison.plot = ggplot(metadata.lsarp.uc.stool[,c("HM", "ave.fiber", "group")] %>% distinct(),
  ...
  annotate(geom="text", y=Inf, x=1.5, vjust=1.5, size=4,
           label=paste("𝘱: ", round(lsarp.uc.fiber.mean.pval, digits=3), sep=""))+
  theme_classic()+theme(legend.position="none",
                        strip.text=element_text(size=10))+
...
save(
  ...
  lsarp.uc.asv.median.glom,
  # destination
  file = "./2026_03_03_lsarp_uc_data.Renv")
...
lsarp.uc.stool.plot = ggplot(metadata.lsarp.uc.stool %>% subset(!HM %in% excluded.lsarp.uc) %>%
                               subset(standard.name != "HM0940-STL-03") %>%
                               # remove post-flare
                               subset(lsarp.days <= flare_day | is.na(flare_day)),
                             aes(x=lsarp.days, y=reorder(HM, flare_day)))+
  ...
  geom_point(fill = "white", aes(shape=phase), size=5)+
  # then plot real points (they overlap in the window, but look good when saved at full size)
  geom_point(aes(fill=phase, shape=phase), color="white",
             #      alpha = ifelse(lsarp.days <= exit.day, "1", "0")),
             size=5)+
  scale_fill_manual(values=c(2,"grey","grey"))+
  scale_shape_manual(values=c(23, 21, 21))+
  # add flare date
  geom_point(data=metadata.lsarp.uc.stool[,c("flare_day", "lsarp.days","HM", "group")] %>% 
               subset(!HM %in% excluded.lsarp.uc) %>% distinct(),
             aes(x=flare_day, y=reorder(HM, (lsarp.days))), shape=4, size=5)+
...
lsarp.uc.rs.selections.rs.plot <- ggplot(subset(lsarp.uc.rs.selections,group=="RS"),
  ...
  facet_wrap(~HM, nrow=3,
             labeller = labeller(HM = setNames(lsarp.uc.deid$code, lsarp.uc.deid$HM)))+
...
lsarp.uc.coxph = survival::coxph(Surv(time, event) ~ group, data = lsarp.uc.survival.data)
lsarp.uc.coxph <- broom::tidy(lsarp.uc.coxph, exponentiate = TRUE, conf.int = TRUE) %>%
...
lsarp.uc.survival.plot.pp = lsarp.uc.survival.plot$plot + 
  ylim(1,0.25)+
  scale_y_reverse(limits = c(1, 0.25), 
                  breaks = c(0,0.25,0.50,0.75),
                  labels = scales::percent(rev(seq(0, 0.75, by = 0.25))))+
  labs(title="Per-protocol (PP)")+theme(plot.title = element_text(hjust = 0.5))

lsarp.uc.survival.pp.plots = lsarp.uc.survival.plot.pp/
  lsarp.uc.survival.plot$table+#labs(title=NULL)+
  patchwork::plot_layout(heights=c(4,1))

lsarp.uc.survival.pp.plots

lsarp.uc.coxph #          HR = 1.60 (0.357 - 7.162)
lsarp.uc.logrank$pvalue # p = 0.536


# :: BASELINE -------------------------------------------------------------

lsarp.uc.baseline = read.csv("./2026_08_05_lsarp_uc_baseline.csv")

# clean
vars = c("age_at_endo", "bmi_at_endo", "M", "left_sided..E2.", "extensive..E3.",
         "pancolitis..E4.", "pucai_at_rs", "pucai", "MES", "oral_pred", "oral_mes",
         "iv_cs", "rectal_mes", "oral_mes_main", "aza_main", "crp", "fcal")

# remove HMs
lsarp.uc.baseline = subset(lsarp.uc.baseline, !HM %in% c("HM0923", "HM0929", "HM0908", "HM0990"))

# run through stats (wilcoxon vs Fishers)
lsarp.uc.baseline.stats = do.call(rbind, lapply(vars, function(x){
  print(x)
  # for numerical, use wilcoxon
  if(x %in% c("age_at_endo","pucai_at_rs", "bmi_at_endo", "pucai", "MES", "crp", "fcal")){
    # select columns
    data.subset = lsarp.uc.baseline[, colnames(lsarp.uc.baseline) %in% c("Group", x)]
...
lsarp.uc.baseline.stats

write.csv(lsarp.baseline.stats,
          "~/Documents/PhD/From_Dave/2026_08_05_lsarp_uc_baseline_stats.csv")

# also check PUCAI at baseline

lsarp.uc.pucai = lsarp.uc.baseline[, colnames(lsarp.uc.baseline) %in% c("Group", "pucai_at_rs")]
wilcox.test(subset(lsarp.uc.pucai, Group == "RS")$pucai_at_rs,
            subset(lsarp.uc.pucai, Group == "Plac")$pucai_at_rs)
# p = 0.661
...
lsarp.uc.fcal.plot = ggplot(subset(metadata.lsarp.uc.stool, !is.na(fecalcal_res)),
  ...
  geom_text(data=pval.positioner("fecalcal_res",
                                 metadata.lsarp.uc.stool,
                                 data1=lsarp.uc.stats.fcal.treatment,
                                 data2=lsarp.uc.stats.fcal.washout,
                                 type="group",
                                 yadjust=2), 
...
lsarp.uc.stats.richness.washout = lmerTest::lmer(scale(log10(richness)) ~ group*scale(lsarp.days) + ave.fiber +  (1|HM),
  ...
  lsarp.uc.richness.plot = ggplot(subset(metadata.lsarp.uc.stool.asv, !is.na(richness)),
    ...
    geom_text(data=pval.positioner("richness",
                                   metadata.lsarp.uc.stool.asv,
                                   data2=lsarp.uc.stats.richness.washout,
                                   data1=lsarp.uc.stats.richness.treatment,
                                   type="group",
                                   yadjust=1.05), 
              aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
...
# :: :: Shannon -----------------------------------------------------------
  ...
  lsarp.uc.shannon.plot = ggplot(subset(metadata.lsarp.uc.stool.asv, !is.na(shannon)),
    ...
    geom_text(data=pval.positioner("shannon",
                                   metadata.lsarp.uc.stool.asv,
                                   data2=lsarp.uc.stats.shannon.washout,
                                   data1=lsarp.uc.stats.shannon.treatment,
                                   type="group",
                                   yadjust=1.05), 
              aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
  ...
  lsarp.uc.but.i.plot = ggplot(subset(metadata.lsarp.uc.stool.asv, !is.na(but.i)),
    ...
    geom_text(data=pval.positioner("but.i",
                                   metadata.lsarp.uc.stool.asv,
                                   data2=lsarp.uc.stats.but.i.washout,
                                   data1=lsarp.uc.stats.but.i.treatment,
                                   type="group",
                                   yadjust=1.25), 
  ...
  lsarp.uc.but.ii.plot = ggplot(subset(metadata.lsarp.uc.stool.asv, !is.na(but.ii)),
    ...
    geom_text(data=pval.positioner("but.ii",
                                   metadata.lsarp.uc.stool.asv,
                                   data2=lsarp.uc.stats.but.ii.washout,
                                   data1=lsarp.uc.stats.but.ii.treatment,
                                   type="group",
                                   yadjust=1.25), 
              aes(x=xval, y=yval*100, label=label), vjust=1,size=4)+
    facet_wrap(~group,scales="free_x")+
  ...
  lsarp.uc.fr.plot = ggplot(subset(metadata.lsarp.uc.stool.asv, !is.na(fr)),
    ...
    geom_text(data=pval.positioner("fr",
                                   metadata.lsarp.uc.stool.asv,
                                   data2=lsarp.uc.stats.fr.washout,
                                   data1=lsarp.uc.stats.fr.treatment,
                                   type="group",
                                   yadjust=1.0), 
              aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group,scales="free_x")+
  ...
  lsarp.uc.fd.plot = ggplot(subset(metadata.lsarp.uc.stool.asv, !is.na(fd)),
    ...
    geom_text(data=pval.positioner("fd",
                                   metadata.lsarp.uc.stool.asv,
                                   data2=lsarp.uc.stats.fd.washout,
                                   data1=lsarp.uc.stats.fd.treatment,
                                   type="group",
                                   yadjust=1.01), 
              aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group,scales="free_x")+
  ...
  lsarp.uc.load.plot = ggplot(subset(metadata.lsarp.uc.stool.asv, !is.na(load.asv)),
    ...
    geom_text(data=pval.positioner("load.asv",
                                   metadata.lsarp.uc.stool.asv,
                                   data2=lsarp.uc.stats.load.washout,
                                   data1=lsarp.uc.stats.load.treatment,
                                   type="group",
                                   yadjust=1.0), 
              aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group,scales="free_x")+
...
# :: :: Bray-Curtis -------------------------------------------------------
  ...
  lsarp.uc.asv.median.glom = lsarp.uc.asv.median.glom[,colSums(lsarp.uc.asv.median.glom.pa) > nrow(lsarp.uc.asv.median.glom.pa)*0.2]
  ...
  lsarp.uc.asv.pcoa.plot <- ggplot(
    ...
    labs(x=paste("Axis 1: ", round(unique(lsarp.uc.asv.pcoa.df$var1), digits=2), "%", sep=""), 
         ...
         title=paste(paste("Treatment R²: ", round(data.frame(lsarp.uc.asv.permanova)[2,3], 3)*100, "%",
                           "  𝘱: ", round(data.frame(lsarp.uc.asv.permanova)[2,5], 3), sep="")), sep="") + 
    facet_wrap(~group)+
    theme_classic()+theme(legend.position="none",
                          plot.title = element_text(hjust = 0.5, size=12),
                          strip.text = element_text(size=10),
...
# :: :: Beta-Diversity -------------------------------------------------------
  ...
  metadata.lsarp.uc.stool.asv.between.plot = ggplot(beta.trajectory.data %>%
    ...
    geom_text(data=pval.positioner("between.beta",
                                   ...
                                   data2=stats.beta.between.washout,
                                   data1=stats.beta.between.treatment,
                                   type="group",
                                   yadjust=1.0), 
              aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
  ...
  metadata.lsarp.uc.stool.asv.within.plot = ggplot(beta.trajectory.data %>%
    ...
    geom_text(data=pval.positioner("within.beta",
                                   beta.trajectory.data,
                                   data2=stats.beta.within.washout,
                                   data1=stats.beta.within.treatment,
                                   type="group",
                                   yadjust=1.0), 
              aes(x=xval, y=yval, label=label), vjust=1,size=4)+
    facet_wrap(~group)+
...
# fix names
  lsarp.uc.asv.lmer$feature =  ifelse(grepl("s__", (lsarp.uc.asv.lmer$taxa)), paste("(s) ", gsub("g__", "", gsub("f__", "", gsub("s__", "", (lsarp.uc.asv.lmer$taxa)))), sep=""),
...
# :: :: Heatmaps  -------------------------------------------------------
  ...
  lsarp.uc.asv.lmer.heatmap = lsarp.uc.asv.median.tax[,colnames(lsarp.uc.asv.median.tax) %in% 
                                                           subset(lsarp.uc.asv.lmer, pval<0.1 & phase == "treatment")$taxa]
  ...
  # Feature annotations
  lsarp.uc.asv.lmer.heatmap.feature.map = subset(lsarp.uc.asv.lmer, pval<0.1 & phase == "treatment")
  lsarp.uc.asv.lmer.heatmap.feature.map = lsarp.uc.asv.lmer.heatmap.feature.map[,c("feature", "estimate", "pval")]
  rownames(lsarp.uc.asv.lmer.heatmap.feature.map) = lsarp.uc.asv.lmer.heatmap.feature.map$feature
  lsarp.uc.asv.lmer.heatmap.feature.map$feature = NULL
  colnames(lsarp.uc.asv.lmer.heatmap.feature.map) = c("Interaction", "𝑝 value")
  ...
  lsarp.uc.asv.lmer.heatmap = reshape2::melt(as.matrix(lsarp.uc.asv.lmer.heatmap)) %>%
    ...
    reshape2::acast(Var2 ~ Var1, value.var="slfc") %>% t() %>% data.frame()
  
  # fix names
  colnames(lsarp.uc.asv.lmer.heatmap) =  ifelse(grepl("s__", colnames(lsarp.uc.asv.lmer.heatmap)), paste("(s) ", gsub("g__", "", gsub("f__", "", gsub("s__", "", colnames(lsarp.uc.asv.lmer.heatmap)))), sep=""),
                               paste("(", substr(colnames(lsarp.uc.asv.lmer.heatmap), 1, 1), ") ", gsub("g__", "", gsub("f__", "", gsub("s__", "", colnames(lsarp.uc.asv.lmer.heatmap)))), sep=""))

  # lfc heatmap
  pheatmap::pheatmap(t(lsarp.uc.asv.lmer.heatmap),
                     #scale_rows=T, # 
                     clustering_distance_rows = "correlation",
                     clustering_distance_cols = "correlation",
                     fontsize_row = 7,
                     ...
                     annotation_colors = list(`Group` = c(Placebo = gg_color_hue(2)[1],
                                              ...
                                              `𝑝 value` = colorRampPalette(c("white","lightblue", "blue"))(100)))
...
# >> Plots ----------------------------------------------------------------
  ...
  # Stools
  ((lsarp.uc.stool.plot + lsarp.uc.survival.pp.plots)+
    patchwork::plot_layout(nrow=1 ,widths=c(2,1)))%>%
    ggsave(filename="./lsarp_plots_deidentified/2026_07_28_lsarp_uc_stool_km_curve_plot_1.pdf",
  ...
  (lsarp.uc.rs.selections.placebo.plot+
    ...
    lsarp.uc.rs.selections.frequences.plot+
    patchwork::plot_layout(widths=c(4,2,2)))%>%
  ggsave(filename="./lsarp_plots_deidentified/2026_01_08_lsarp_uc_rs_selections_2.pdf",
  ...
  (lsarp.uc.but.i.plot + 
    ...
    ggsave(filename="./lsarp_plots/2026_03_01_lsarp_uc_variables_3.pdf",
           width=14, height=8, device = cairo_pdf)
  
  # Bray-Curtis + Volcano
    (lsarp.uc.asv.pcoa.plot/
      lsarp.uc.asv.lmer.interactions.plot) %>%
      ggsave(filename="./lsarp_plots_deidentified/2026_01_08_lsarp_uc_pcoa_omics_4.pdf",
  ...
  pheatmap::pheatmap(t(lsarp.uc.asv.lmer.heatmap),
                     ...
                     breaks=c(seq(min(lsarp.uc.asv.lmer.heatmap), 0, length.out=ceiling(100/2) + 1), 
                              seq(max(lsarp.uc.asv.lmer.heatmap)/100, max(lsarp.uc.asv.lmer.heatmap), length.out=floor(100/2))),
                     annotation_col=lsarp.uc.asv.lmer.heatmap.hm.map,
                     labels_col = sapply(rownames(lsarp.uc.asv.lmer.heatmap), deid.fun),
                     annotation_row = lsarp.uc.asv.lmer.heatmap.feature.map,
                     annotation_colors = list(`Group` = c(Placebo = gg_color_hue(2)[1],
                                                          RS = gg_color_hue(2)[2]),
                                              `Interaction` = colorRampPalette(c("blue","white", "red"))(100),
    ...
    ggsave(filename="./lsarp_plots_deidentified/2026_03_01_lsarp_uc_heatmap_5.pdf",
...
# >> Supplementals --------------------------------------------------------
  ...
  subset(lsarp.uc.asv.lmer, pval <= 0.1)
  lsarp.uc.asv.median.glom
  
  lsarp.uc.asv.heatmap.data.treatment = ((lsarp.uc.asv.median.glom[rownames(lsarp.uc.asv.median.glom) %in% subset(metadata.lsarp.uc.stool.asv, phase %in% c("preRS", "onRS"))$standard.name,
                                                          colnames(as.data.frame(lsarp.uc.asv.median.glom)) %in% 
                                                            subset(lsarp.uc.asv.lmer,phase == "treatment" & pval <= 0.1)$taxa])/500 + 
    min(lsarp.uc.asv.median.glom[lsarp.uc.asv.median.glom!=0])/1000) %>%
    log10() %>% as.matrix() %>% scale() %>% t() 
  
  lsarp.uc.asv.heatmap.data.washout = ((lsarp.uc.asv.median.glom[rownames(lsarp.uc.asv.median.glom) %in%subset(metadata.lsarp.uc.stool.asv, phase == "postRS" | rs.end == "rs.end")$standard.name,
                                                        colnames(as.data.frame(lsarp.uc.asv.median.glom)) %in% 
                                                          subset(lsarp.uc.asv.lmer, phase == "washout" & pval <= 0.1)$taxa])/500 + 
    min(lsarp.uc.asv.median.glom[lsarp.uc.asv.median.glom!=0])/1000) %>%
    log10() %>% as.matrix() %>% scale() %>%  # scaled per column
    t() 
  
  pheatmap::pheatmap((lsarp.uc.asv.heatmap.data.treatment),
                     color=colorRampPalette(c("blue","white", "red"))(100),
                     clustering_distance_rows = "correlation",
                     clustering_distance_cols = "correlation",
                     breaks=c(seq(min(lsarp.uc.asv.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                              seq(max(lsarp.uc.asv.heatmap.data.treatment)/100, max(lsarp.uc.asv.heatmap.data.treatment), length.out=floor(100/2))),
                     annotation_row = subset(lsarp.uc.asv.lmer,phase == "treatment" & pval <= 0.1)%>%
                     ...
                     annotation_col = subset(metadata.lsarp.uc.stool, phase %in% c("preRS", "onRS"))[,c("HM", "lsarp.days", "group", "rs.col", "ave.fiber", "standard.name")] %>%
                      mutate(RS_Name = as.character(rs.col)) %>%
                       mutate(RS_Name = ifelse((RS_Name) %in% (rs.names), RS_Name, NA)) %>%
                       ...
                       mutate(Group = group,
                              `Days on product` = as.numeric(lsarp.days),
                              Fiber = ave.fiber) %>% dplyr::select(Group, `Days on product`, Fiber, RS_Type),
                     annotation_colors = lsarp.omic.heatmap.mapping.1,
                     labels_col = sapply(colnames(lsarp.uc.asv.heatmap.data.treatment), deid.fun),
                     labels_row = name.shortener(rownames(lsarp.uc.asv.heatmap.data.treatment))$new,
                     fontsize_row = 10 - nrow(lsarp.uc.asv.heatmap.data.treatment) / 15,
                     fontsize_col = 7,
                     main = "ASV | Treatment Phase",
                     border_color = "white") %>%
    ggsave(filename="./lsarp_plots_deidentified/lsarp_uc_group_asv_tr_heatmap.pdf",
           device = cairo_pdf,
           width=16, height=12)
  
  
  
  pheatmap::pheatmap((lsarp.uc.asv.heatmap.data.washout),
                     color=colorRampPalette(c("blue","white", "red"))(100),
                     clustering_distance_rows = "correlation",
                     clustering_distance_cols = "correlation",
                     breaks=c(seq(min(lsarp.uc.asv.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                              seq(max(lsarp.uc.asv.heatmap.data.washout)/100, max(lsarp.uc.asv.heatmap.data.washout), length.out=floor(100/2))),
                     annotation_row = subset(lsarp.uc.asv.lmer,phase == "washout" & pval <= 0.1)%>%
                       mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                       mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                       tibble::remove_rownames() %>% 
                       tibble::column_to_rownames("taxa") %>%
                       ...
                       mutate(Coefficient = estimate,
                              Significance = sig) %>% dplyr::select(Coefficient, Significance),
                     annotation_col = subset(metadata.lsarp.uc.stool, phase == "postRS" | rs.end == "rs.end")[,c("HM", "lsarp.off", "group", "rs.col", "ave.fiber", "standard.name")] %>%
                       mutate(RS_Name = as.character(rs.col)) %>%
                       mutate(RS_Name = ifelse((RS_Name) %in% (rs.names), RS_Name, NA)) %>%
                       merge(rs.type.code, all.x = T) %>%
                       tibble::column_to_rownames("standard.name") %>%
                       mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                       mutate(Group = group,
                              `Days off product` = as.numeric(lsarp.off),
                              Fiber = ave.fiber) %>% dplyr::select(Group, `Days off product`, Fiber, RS_Type),
                     annotation_colors = lsarp.omic.heatmap.mapping.2,
                     labels_col = sapply(colnames(lsarp.uc.asv.heatmap.data.washout), deid.fun),
                     labels_row = name.shortener(rownames(lsarp.uc.asv.heatmap.data.washout))$new,
                     fontsize_row = 10 - nrow(lsarp.uc.asv.heatmap.data.washout) / 15,
                     fontsize_col = 7,
                     main = "ASV | Washout Phase",
                     border_color = "white") %>%
    ggsave(filename="./lsarp_plots_deidentified/lsarp_uc_group_asv_wo_heatmap.pdf",
           device = cairo_pdf,
           width=16, height=12)
...
# :: Tables ---------------------------------------------------------------
  ...
  lsarp.uc.asv.lmer %>% subset(phase == "treatment")
  lsarp.uc.asv.lmer %>% subset(phase == "washout")

  
  openxlsx::write.xlsx(
    list(`st 35 uc_asv_tr` = lsarp.uc.asv.lmer %>% subset(phase == "treatment") %>% mutate(data.type = "ASV", diagnosis="UC"),
        `st 36 uc_asv_wo` = lsarp.uc.asv.lmer %>% subset(phase == "washout") %>% mutate(data.type = "ASV", diagnosis="UC")),
    "lsarp_paper/lsarp_uc_heatmap_source.xlsx"
  )
